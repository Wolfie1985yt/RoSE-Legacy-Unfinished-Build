...
function onCreate()
	if getPropertyFromClass('backend.ClientPrefs', 'PotatoOptimization') == false then
		makeAnimatedLuaSprite('sky', 'bgs/jp/sky', -150, -50);
		addAnimationByPrefix('sky', 'idle', 'idle', 24, true);
		scaleObject('sky', 2.8, 2.6);
		setScrollFactor('sky', 0.2, 0.5);
		setProperty('sky.antialiasing', false);
		addLuaSprite('sky', false);
		
		makeLuaSprite('city', 'bgs/jp/city', -150, 0);
		scaleObject('city', 1.2, 1.2);
		setScrollFactor('city', 0.5, 0.7);
		setProperty('city.antialiasing', true);
		addLuaSprite('city', false);
		
		makeLuaSprite('ground', 'bgs/jp/ground', -150, -100);
		scaleObject('ground', 1.4, 1.2);
		setScrollFactor('ground', 1, 1);
		setProperty('ground.antialiasing', true);
		addLuaSprite('ground', false);
		
		makeAnimatedLuaSprite('commercial', 'bgs/jp/commercial', 820, 230);
		addAnimationByPrefix('commercial', 'idle', 'commercial idle', 30, true);
		scaleObject('commercial', 0.7, 1.05);
		setScrollFactor('commercial', 1, 1);
		setProperty('commercial.antialiasing', false);
		setObjectOrder('commercial', 'ground');
...
function onUpdate()
	...
	if songName == "endless-jp" then
		...
		if curStep == 1158 then
			...
			yy2 = 600;
			if getPropertyFromClass('backend.ClientPrefs', 'PotatoOptimization') == false then
				addLuaSprite('commercial', false);
				playAnim('commercial', 'idle', true)
