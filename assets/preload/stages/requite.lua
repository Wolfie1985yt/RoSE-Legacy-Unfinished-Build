...
function onCreate()
	if getPropertyFromClass('backend.ClientPrefs', 'PotatoOptimization') == false then
		...
		makeLuaSprite('easteregg', 'bgs/fd-bg/classic/easteregg', 0, 60);
		scaleObject('easteregg', 2.00, 2.00);
		setProperty('easteregg.antialiasing', true);
		setProperty('easteregg.visible',false);
		addLuaSprite('easteregg', false);
...
function goodNoteHit(index, noteDir, noteType, isSustainNote)
	if noteType == 'Bullet Note' then
		...
		if getRandomInt(1,2) == 1 then
			triggerEvent('Play Animation', 'shoot1', 'bf')
		else
			triggerEvent('Play Animation', 'shoot2', 'bf')
		end
	end
	if noteType == 'Knife Note' then
		debugPrint(prevHealth)
		limitHealth = true;
		prevHealth = prevHealth - 0.4;
		maxHealth = prevHealth - 0.4;
		runTimer('healthReset',15);
	end
end
function onUpdate(elapsed)
	if limitHealth == true then
...
function secretWordTyped()
	setProperty('easteregg.visible',true);
end