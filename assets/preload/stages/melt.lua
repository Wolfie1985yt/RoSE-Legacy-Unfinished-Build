...
function onCreate()
	...
	addAnimationByPrefix('gfMelthog', 'sad', 'Miss', 24, false);
	...
	makeAnimatedLuaSprite('glitch', 'bgs/melt/Glitch', -1800, -3400);
	addAnimationByPrefix('glitch', 'idle', 'Glitch', 24, true);
	scaleObject('glitch', 6, 6);
	setScrollFactor('glitch', 1, 1);
	setProperty('glitch.antialiasing', false);
	setObjectOrder('glitch', 'blackscreen');
	...
	runHaxeCode([[
		...
		CloudNightSix = new FlxBackdrop(Paths.image('bgs/melt/night/clouds/6'), 1, 1, true, false);
...
function onEvent(event, val1, val2)
	if event == 'Change Character' then
		if val1 == "1" or val1 == 'dad' or val1 == 'opponent' then
			if val2 == 'Melthog_0' then
				objectPlayAnimation('tvIcons', 'melthog0', true)
			end
...
function onStepHit()	
	...
	if songName == "melting" or songName == "Melting" then
		...
		if curStep == 2240 then
			doTweenAlpha('camHUDTween5','camHUD',0,2.5,'linear')
		end
		...
		if curStep == 2361 then
			triggerEvent('Change Character', 'dad', 'Melthog_7_Transform');
...
function onBeatHit()
	if curBeat % 2 == 0 and leftAnim == true then
		objectPlayAnimation('gfMelthog', 'left', false);
		leftAnim = false;
		rightAnim = true
	elseif curBeat % 2 == 0 and rightAnim == true then
