...
function onStepHit()
	if curStep == 1960 and getPropertyFromClass('backend.ClientPrefs', 'PotatoOptimization') == false then
		doTweenAlpha('darkness','black',1,0.001,'linear')
	end
	if curStep == 1964 and getPropertyFromClass('backend.ClientPrefs', 'PotatoOptimization') == false then
