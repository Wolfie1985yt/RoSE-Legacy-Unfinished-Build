...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	function reloadText() {
		...
		daText.x = objects.DialogueBoxPsych.DEFAULT_TEXT_X;
		daText.y = objects.DialogueBoxPsych.DEFAULT_TEXT_Y;
		hudGroup.add(daText);
	}
	...
	function reloadCharacter() {
		...
		character.x = objects.DialogueBoxPsych.LEFT_CHAR_X;
		character.y = objects.DialogueBoxPsych.DEFAULT_CHAR_Y;

		switch(character.jsonFile.dialogue_pos) {
			case 'right':
				character.x = FlxG.width - character.width + objects.DialogueBoxPsych.RIGHT_CHAR_X;
	...
	function updateTextBox() {
		...
		objects.DialogueBoxPsych.updateBoxOffsets(box);
	}
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(FlxG.keys.justPressed.ESCAPE) {
				...
				FlxG.sound.playMusic(Paths.music('storymodemenumusic'), 1);
				transitioning = true;
			}
