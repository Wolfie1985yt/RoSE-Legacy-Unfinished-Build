...
class CharacterEditorState extends MusicBeatState
{
	...
	function reloadBGs() {
		...
		var bg:BGSprite = new BGSprite('stageback', 'shared', -600 + OFFSET_X - playerXDifference, -300, 0.9, 0.9);
		bgLayer.add(bg);

		var stageFront:BGSprite = new BGSprite('stagefront', 'shared', -650 + OFFSET_X - playerXDifference, 500, 0.9, 0.9);
	...
	var TemplateCharacter:String = '{
			...
			"image": "characters/DaddyDearest/DADDY_DEAREST",
			"position": [
				0,
				0
			],
	...
	function addCharacterUI() {
		...
		imageInputText = new FlxUIInputText(15, 30, 200, 'characters/BF/HD/BOYFRIEND', 8);
		...
		if(!charDropDown.dropPanel.visible) {
			if (FlxG.keys.justPressed.ESCAPE) {
				if(goToPlayState) {
					...
					if (ClientPrefs.VisibleMouse) FlxG.mouse.visible = true;
				}
				return;
			}
