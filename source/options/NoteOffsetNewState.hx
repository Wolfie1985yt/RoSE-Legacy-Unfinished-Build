...
class NoteOffsetNewState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		arrows = new FlxSprite(682, 75);
		arrows.scale.x = 0.82;
		arrows.scale.y = 0.8;
		arrows.alpha = 0.8;
		arrows.loadGraphic(Paths.image('arrows/delay-adjust/arrows', 'shared'));	
		arrows.updateHitbox();
		...
		if(!ClientPrefs.lowQuality) {
			...
			stageLight1.setGraphicSize(Std.int(stageLight1.width * 1.1));
			stageLight1.loadGraphic(Paths.image('stage_light', 'shared'));	
			stageLight1.antialiasing = ClientPrefs.globalAntialiasing;
			stageLight1.updateHitbox();
			stageLight1.cameras = [camGame];
			add(stageLight1);
		
			stageLight2 = new FlxSprite(1225, -100);
			stageLight2.setGraphicSize(Std.int(stageLight2.width * 1.1));
			stageLight2.loadGraphic(Paths.image('stage_light', 'shared'));
			stageLight2.updateHitbox();
			stageLight2.flipX = true;
			stageLight2.cameras = [camGame];
			stageLight2.antialiasing = ClientPrefs.globalAntialiasing;
			add(stageLight2);
		...
		// Characters
		gf = new Character(300, 30, 'gf');
		gf.x += gf.positionArray[0];
		gf.y += gf.positionArray[1];
		gf.scrollFactor.set(0.95, 0.95);
		boyfriend = new Character(770, 0, 'bf', true);
		boyfriend.x += boyfriend.positionArray[0];
		boyfriend.y += boyfriend.positionArray[1];
		dad = new Character(-120, -50, 'xenofakes', false);
	...
	function updateMode()
	{
		...
		if(onComboMenu)
			changeModeText.text = '< Click and drag to change where the combo goes >\n(Press Accept to switch to Audio Delay)';
		else
			changeModeText.text = '< Press left/right to change the audio delay >\n(Press Accept to Switch to combo offset)';
