...
class OptionsNewState extends MusicBeatState
{
	...
	override function create()
	{
		...
		contr = new FlxSprite(332, 67).loadGraphic(Paths.image('options/main/buttons/controls'));
		contr.antialiasing = ClientPrefs.globalAntialiasing;
		contr.scrollFactor.set();
		add(contr);

		contrSel = new FlxSprite(332, 67).loadGraphic(Paths.image('options/main/buttons/controls-sel'));
		contrSel.antialiasing = ClientPrefs.globalAntialiasing;
		contrSel.scrollFactor.set();
		add(contrSel);
		
		graphics = new FlxSprite(332, 168).loadGraphic(Paths.image('options/main/buttons/graphics'));
		graphics.antialiasing = ClientPrefs.globalAntialiasing;
		graphics.scrollFactor.set();
		add(graphics);
		
		graphicsSel = new FlxSprite(332, 168).loadGraphic(Paths.image('options/main/buttons/graphics-sel'));
		graphicsSel.antialiasing = ClientPrefs.globalAntialiasing;
		graphicsSel.scrollFactor.set();
		add(graphicsSel);
		
		offset = new FlxSprite(332, 270).loadGraphic(Paths.image('options/main/buttons/audio-delay-and-combo'));
		offset.antialiasing = ClientPrefs.globalAntialiasing;
		offset.scrollFactor.set();
		add(offset);
		
		offsetSel = new FlxSprite(332, 270).loadGraphic(Paths.image('options/main/buttons/audio-delay-and-combo-sel'));
		offsetSel.antialiasing = ClientPrefs.globalAntialiasing;
		offsetSel.scrollFactor.set();
		add(offsetSel);

		visandui = new FlxSprite(332, 370).loadGraphic(Paths.image('options/main/buttons/visuals-and-ui'));
		visandui.antialiasing = ClientPrefs.globalAntialiasing;
		visandui.scrollFactor.set();
		add(visandui);
		
		visanduiSel = new FlxSprite(332, 370).loadGraphic(Paths.image('options/main/buttons/visuals-and-ui-sel'));
		visanduiSel.antialiasing = ClientPrefs.globalAntialiasing;
		visanduiSel.scrollFactor.set();
		add(visanduiSel);
		
		gameplay = new FlxSprite(332, 474).loadGraphic(Paths.image('options/main/buttons/gameplay'));
		gameplay.antialiasing = ClientPrefs.globalAntialiasing;
		gameplay.scrollFactor.set();
		add(gameplay);
		
		gameplaySel = new FlxSprite(332, 474).loadGraphic(Paths.image('options/main/buttons/gameplay-sel'));
		gameplaySel.antialiasing = ClientPrefs.globalAntialiasing;
		gameplaySel.scrollFactor.set();
		add(gameplaySel);
		
		hitboxControls = new FlxSprite(380,110).makeGraphic(520,89, 0x00000000);
		add(hitboxControls);
		hitboxGraphics = new FlxSprite(380,211).makeGraphic(520,89, 0x00000000);
		add(hitboxGraphics);
		hitboxOffset = new FlxSprite(380,313).makeGraphic(520,89, 0x00000000);
		add(hitboxOffset);
		hitboxVisAndUI = new FlxSprite(380,413).makeGraphic(520,89, 0x00000000);
		add(hitboxVisAndUI);
		hitboxGameplay = new FlxSprite(380,517).makeGraphic(520,89, 0x00000000);
		add(hitboxGameplay);
	...
	override function update(elapsed:Float)
	{
		...
		if (!locked) {
			...
			if(FlxG.keys.pressed.SHIFT) shiftMult = 4;
				...
				if (FlxG.mouse.overlaps(hitboxOffset) && curSelected != 2) changeSel(2, true, true);
				if (FlxG.mouse.overlaps(hitboxVisAndUI) && curSelected != 3) changeSel(3, true, true);
				if (FlxG.mouse.overlaps(hitboxGameplay) && curSelected != 4) changeSel(4, true, true);
			}
			#end
			...
			if (controls.ACCEPT #if desktop || ((FlxG.mouse.justPressedMiddle || FlxG.mouse.justPressed) && (FlxG.mouse.overlaps(hitboxControls) || FlxG.mouse.overlaps(hitboxGraphics) || FlxG.mouse.overlaps(hitboxOffset) || FlxG.mouse.overlaps(hitboxVisAndUI) || FlxG.mouse.overlaps(hitboxGameplay))) #end) {
				FlxTween.tween(blackScreen, {alpha: 1}, 0.25, {onComplete: function(twn:FlxTween) {
					switch(curSelected) {
						...
						case 2: #if mobile removeVirtualPad(); #end MusicBeatState.switchState(new options.NoteOffsetNewState());
						case 3: #if mobile removeVirtualPad(); #end MusicBeatState.switchState(new options.VisualsUINewState());
						case 4: #if mobile removeVirtualPad(); #end MusicBeatState.switchState(new options.GameplayNewState());
					}
				}});
			}
			#if mobile
