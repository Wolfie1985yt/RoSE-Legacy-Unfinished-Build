...
class ControlsNewState extends MusicBeatState
{
	...
	override function create()
	{
		...
		EMOTE = new FlxText(660, 528, FlxG.width, "EMOTE");
		EMOTE.setFormat(Paths.font("advanced.ttf"), 35, 0xFFFFFFFF, LEFT, FlxTextBorderStyle.OUTLINE, 0xFF262626);
		EMOTE.scrollFactor.set();
		EMOTE.borderSize = 1.25;
		add(EMOTE);
		
		EMOTEF = new FlxText(210, 528, FlxG.width, '[ ' + ClientPrefs.keyBinds.get('emote')[0] + ' ]');
		EMOTEF.setFormat(Paths.font("advanced.ttf"), 35, 0xFFB400FF, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.WHITE);
		EMOTEF.scrollFactor.set();
		EMOTEF.borderSize = 1.25;
		add(EMOTEF);
		
		EMOTES = new FlxText(360, 528, FlxG.width, '[ ' + ClientPrefs.keyBinds.get('emote')[1] + ' ]');
		EMOTES.setFormat(Paths.font("advanced.ttf"), 35, 0xFFB400FF, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.WHITE);
		EMOTES.scrollFactor.set();
		EMOTES.borderSize = 1.25;
		add(EMOTES);
	...
	function Select(change:Int = 0, ?playSound:Bool = true)
	{
		...
		EMOTEF.color = 0xFFB400FF;
		EMOTES.color = 0xFFB400FF;
		...
		switch(curSelected) {
			...
			case 36: EMOTEF.color = 0xFFFFD700;
			case 37: EMOTES.color = 0xFFFFD700;
		}
	}
	...
	function changeBind(?bind:Bool = false)
	{
		...
		if (rebindingKey && changing) {
			...
			EMOTEF.color = 0xFF292929;
			EMOTES.color = 0xFF292929;
			...
			TimerOne = new FlxTimer().start(0.4, function(tmro:FlxTimer) {
				switch(curSelected) {
					...
					case 36: EMOTEF.color = 0xFFB400FF;
					case 37: EMOTES.color = 0xFFB400FF;
				}
	...
	function Reset(?fr:Bool = false, ?time = 1.5)
	{
		...
		if (ResetNooo) {
			...
			EMOTEF.color = 0xFF3D0000;
			EMOTES.color = 0xFF3D0000;
			...
			TimerReset = new FlxTimer().start(time, function(tmrdo:FlxTimer) {
				...
				EMOTEF.color = 0xFFFF0000;
				EMOTES.color = 0xFFFF0000;
	...
	function TextUpd()
	{
		...
		EMOTEF.text = '[ ' + ClientPrefs.keyBinds.get('emote')[0] + ' ]';
		EMOTES.text = '[ ' + ClientPrefs.keyBinds.get('emote')[1] + ' ]';
	}
		...
		if (!locked) {
			if (!rebindingKey) {
				...
				switch(curSelected) {
					...
					case 36: keyPressed = [FlxG.keys.firstJustPressed(), ClientPrefs.keyBinds.get('emote')[1]];
					case 37: keyPressed = [ClientPrefs.keyBinds.get('emote')[0], FlxG.keys.firstJustPressed()];
				}
				...
				if (keyP > -1) {
					switch(curSelected) {
						...
						case 36: ClientPrefs.keyBinds.set('emote', keyPressed);
						case 37: ClientPrefs.keyBinds.set('emote', keyPressed);
					}
					if (TimerOne != null) TimerOne.cancel();
					if (TimerTwo != null) TimerTwo.cancel();
