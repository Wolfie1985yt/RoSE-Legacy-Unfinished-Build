	...
	override function update(elapsed:Float)
	{
		if (control && !pressed) {
			...
			if (controls.BACK || (controls.ACCEPT && Exit) || #if mobile FlxG.android.justReleased.BACK #else (FlxG.mouse.overlaps(No) && FlxG.mouse.justReleased) || FlxG.mouse.justPressedRight || (FlxG.mouse.overlaps(ExitBtn) && FlxG.mouse.justReleased) #end) {
				...
				switch (state) {
					case 'freeplay': states.FreeplayState.goreCloseShit = true;
					case 'story': StoryMenuState.GoreBack = true;
					case 'encore': states.EncoreState.goreCloseShit = true;
					...
					new FlxTimer().start(time, function(tmr:FlxTimer) { 
						if (cutsceneName == 'disabled') {
							...
							switch (toState) {
								...
								case 'rosy-mechanic': openSubState(new substates.RosyMechanicSubstate());
							}
						} else {
							MusicBeatState.switchState(new vlc.Cutscenes(cutsceneName, MP4FileName, WaitTime, EndState, songToPlay, DiscordRPCimg, DiscordRPCtext));
						}
					});
				});
				pressed = true;
