...
class GameOverMelthogSubstate extends MusicBeatSubstate
{
	public static var instance:GameOverMelthogSubstate;
	
	var coolcamera:FlxCamera;
	var coolcamera2:FlxCamera;
	var blackScreen:FlxSprite;
	
	var Melthog:FlxSprite;
	...
	public function new()
	{
		...
		Melthog = new FlxSprite(0, 0).loadGraphic(Paths.image('gameovers/melthog/melthog'));
		Melthog.antialiasing = false;
		Melthog.scrollFactor.set();
		Melthog.screenCenter();
		add(Melthog);
		
		var IASH = new FlxSprite(0, FlxG.height - 25);
		IASH.frames = Paths.getSparrowAtlas('gameovers/melthog/i-am-still-here');
		IASH.animation.addByPrefix('idle', 'shakin', 24, true);
		IASH.antialiasing = false;
		IASH.animation.play('idle', true);
		IASH.screenCenter(X);
		add(IASH);
		...
		FlxG.sound.playMusic(Paths.music('melthog-gameover', 'shared'), 0);
	...
	function endBullshit():Void
	{
		if (!isEnding)
		{
			...
			FlxG.sound.play(Paths.music('melthog-gameover-retry'), 0.7);
