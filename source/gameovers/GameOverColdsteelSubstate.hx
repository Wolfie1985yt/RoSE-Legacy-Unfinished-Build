...
class GameOverColdsteelSubstate extends MusicBeatSubstate
{
	public static var instance:GameOverColdsteelSubstate;
	...
	public function new()
	{
		...
		Cold = new FlxSprite(0, 0).loadGraphic(Paths.image('gameovers/coldsteel'));
		Cold.antialiasing = ClientPrefs.globalAntialiasing;
		Cold.scrollFactor.set();
		Cold.screenCenter();
		add(Cold);
		...
		FlxG.sound.playMusic(Paths.music('coldsteel-gameover', 'shared'), 0);
	...
	function endBullshit():Void
	{
		if (!isEnding)
		{
			...
			FlxG.sound.play(Paths.music('coldsteel-gameover-retry'), 0.7);
