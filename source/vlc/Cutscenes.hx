...
{	
	...
	override function create()
	{
        ...
        FlxG.save.bind('pre-recode', 'rose');
		...
		switch (cutsceneName) {
			case 'Haxe':
				#if windows
				backend.WinPaths.getDesktopFileNames(false, false, false, false, true);
				#end
			...
			case 'Haxe-LangChange':
				...
				new FlxTimer().start(0.1, function(tmr:FlxTimer)
				{	
					MusicBeatState.switchState(new states.PreLaunchSettingsState());
				});
			...
			case 'Fatal Encore':
				...
				#if (windows || mac)
				var HaxeFEIntro:VideoHandler = new VideoHandler();
				HaxeFEIntro.play(Paths.video('HaxeFlixelIntro-fatal-enc'));
				HaxeFEIntro.onEndReached.add(function() {	
					HaxeFEIntro.dispose();
					var SonicFEIntro:VideoHandler = new VideoHandler();
					SonicFEIntro.play(Paths.video('sonicexe-intro-fatal-enc'));
					SonicFEIntro.onEndReached.add(function() {	
						SonicFEIntro.dispose();
				#elseif mobile
			...
			case 'tt-final':
				...
				#end
				FlxG.mouse.visible = false;
				if (!ClientPrefs.cutsceneYouSaw.contains('tt-final')) {
					ClientPrefs.cutsceneYouSaw.push('tt-final');
					ClientPrefs.saveSettings();
				}
				...
				new FlxTimer().start(WaitTime, function(WaitTimer:FlxTimer) {
					...
					Cutscene.play(Paths.video('tt-final'));
					...
					Cutscene.playVideo(Paths.video('tt-final'), false, false, cutsceneName);
			...
			case 'STCodes':
				...
				#end
				FlxG.mouse.visible = false;
				if (!ClientPrefs.cutsceneYouSaw.contains('sound-test-codes')) {
					ClientPrefs.cutsceneYouSaw.push('sound-test-codes');
					ClientPrefs.saveSettings();
				}
				...
				new FlxTimer().start(WaitTime, function(WaitTimer:FlxTimer) {
					#if (windows || mac)
					var Cutscene:VideoHandler = new VideoHandler();
					Cutscene.play(Paths.video('sound-test-codes'));
					Cutscene.onEndReached.add(function() {	
						Cutscene.dispose();
						Cutscene.stop();
					...
					#elseif mobile
					var Cutscene:MP4Handler = new MP4Handler();
					Cutscene.playVideo(Paths.video('sound-test-codes'), false, false, cutsceneName);
					Cutscene.finishCallback = function() {	
					#end
					#if (windows || mac)
					}, true);
			...
			default:
				...
				new FlxTimer().start(WaitTime, function(WaitTimer:FlxTimer) {
					#if (windows || mac)
					var Cutscene:VideoHandler = new VideoHandler();
					Cutscene.play(Paths.video(MP4FileName));
					Cutscene.onEndReached.add(function() {	
						Cutscene.dispose();
						Cutscene.stop();
					#elseif mobile
						...
						switch (songToPlay) {
							case 'none':
								switch(EndState) {
									...
									case 'Encore': MusicBeatState.switchState(new states.EncoreState());
	...
	function PlayCutscene(Name:String)
	{
		switch(Name) {
			...
			case 'STCodes':
				...
				{	
					...
					#if (windows || mac)
					video = new VideoHandler();
					video.play(Paths.video('sound-test-codes'));
					video.volume = 300;
					video.onEndReached.add(function() {
					#elseif mobile
					var video:MP4Handler = new MP4Handler();
			...
			case 'tt-final':
				...
				new FlxTimer().start(0.3, function(tmr:FlxTimer)
				{
					...
					#if (windows || mac)
					video = new VideoHandler();
					video.play(Paths.video('tt-final'));
					video.volume = 300;
					video.onEndReached.add(function() {	
						video.dispose();
						#end
			...
			case 'SEGA':
				...
				{	
					if (ClientPrefs.storyProgressExe == 3) {
						#if (windows || mac)
						video = new VideoHandler();
						video.play(Paths.video('sonicexe-intro-fe'));
						video.volume = 300;
						video.onEndReached.add(function() {
						#elseif mobile
						var video:MP4Handler = new MP4Handler();
						...
						#if (windows || mac)
						video = new VideoHandler();
						video.play(Paths.video('sonicexe-intro'));
						video.volume = 300;
						#elseif mobile
						var video:MP4Handler = new MP4Handler();
						...
						video.finishCallback = function() {
							...
							ClientPrefs.StartVideoSaw = true;
							ClientPrefs.saveSettings();
						#if (windows || mac)
						}, true);
						#elseif mobile
		...
		new FlxTimer().start(0.5, function(startTalk:FlxTimer) {
			trace(cutsceneName);
		});
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT && cutsceneName == 'Haxe' && ClientPrefs.StartVideoSaw || FlxG.mouse.justPressed && cutsceneName == 'Haxe' && ClientPrefs.StartVideoSaw) {
		...
		if (ClientPrefs.SongsCompleted.contains('final-escape') && ClientPrefs.storyProgressExe <= 4 && ClientPrefs.vineboom) {
			new FlxTimer().start(0.5, function(songtimer:FlxTimer) {
				ClientPrefs.vineboom = false;
				ClientPrefs.saveSettings();
			});
		}
		if (!ClientPrefs.youSawSoundTestCutscene && ClientPrefs.SongsCompleted.contains('goddess')) {
			new FlxTimer().start(0.5, function(songtimer:FlxTimer) {
				ClientPrefs.vineboom = true;
				ClientPrefs.saveSettings();
			});
