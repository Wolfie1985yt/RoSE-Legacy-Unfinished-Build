...
class Paths
{
	...
	static public function video(key:String)
	{
		...
		#if android
		trace('${Sys.getCwd()}assets/videos/$key.$VIDEO_EXT');
	...
	static public function truvideo(key:String)
	{
		return 'assets/videos/$key.$VIDEO_EXT';
	}
	...
	{
		var langKey:String = key;
		var langLib:String = library;
		...
		if (ClientPrefs.Language != 'English') {
			...
			if (TranslationLanguages.Files.contains(key)) {
				langKey = 'language-translation/' + lang + key;
				langLib = 'preload';
			}
		}
		
		var returnAsset:FlxGraphic = returnGraphic(langKey, langLib, allowGPU);
		return returnAsset;
	}
	
	static public function getTextFromFile(key:String, ?ignoreMods:Bool = false):String
	{
		#if desktop //androi pt
		#if MODS_ALLOWED
		...
		{
			var levelPath:String = '';
			if(currentLevel != 'shared' || currentLevel != 'exe') {
	...
	inline static public function getSparrowAtlas(key:String, ?library:String, ?allowGPU:Bool = true):FlxAtlasFrames
	{
		...
		#if MODS_ALLOWED
		var imageLoaded:FlxGraphic = returnGraphic(langKey, langLib, allowGPU);
		var xmlExists:Bool = false;
		if(FileSystem.exists(modsXml(langKey))) {
			xmlExists = true;
		}

		return FlxAtlasFrames.fromSparrow((imageLoaded != null ? imageLoaded : image(langKey, langLib, true)), (xmlExists ? File.getContent(modsXml(langKey)) : file('images/$langKey.xml', langLib)));
		#else
		return FlxAtlasFrames.fromSparrow(image(langKey, langLib, true), file('images/$langKey.xml', langLib));
		#end
	}
	...
	inline static public function getPackerAtlas(key:String, ?library:String, ?allowGPU:Bool = true)
	{
		...
		return FlxAtlasFrames.fromSpriteSheetPacker((imageLoaded != null ? imageLoaded : image(key, library, true)), (txtExists ? File.getContent(modsTxt(key)) : file('images/$key.txt', library)));
		#else
	...
	public static function returnGraphic(key:String, ?library:String, ?allowGPU:Bool = true) {
		...
		if(FileSystem.exists(modKey)) {
			if(!currentTrackedAssets.exists(modKey)) {
				bitmap = BitmapData.fromFile(modKey);
				if (allowGPU && ClientPrefs.GPUCaching && bitmap.width * bitmap.height > 2073600) {
					...
					trace('Sprite Loaded in GPU: ' + getPath('images/$key.png', IMAGE, library));
				} else {
				    newGraphic = FlxGraphic.fromBitmapData(bitmap, false, modKey);
					trace('Sprite Loaded in RAM: ' + getPath('images/$key.png', IMAGE, library));
				}
		...
		trace('Sprite not found: ' + getPath('images/$key.png', IMAGE, library));
		return null;
	}
