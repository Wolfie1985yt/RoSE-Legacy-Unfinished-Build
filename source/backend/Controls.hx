...
enum abstract Action(String) to String from String
{
	...
	var NOTE_RING = "note_ring";
	var NOTE_UP_P = "note_up-press";
	var NOTE_LEFT_P = "note_left-press";
	var NOTE_RIGHT_P = "note_right-press";
	var NOTE_DOWN_P = "note_down-press";
	var NOTE_RING_P = "note_ring-press";
	var NOTE_UP_R = "note_up-release";
	var NOTE_LEFT_R = "note_left-release";
	var NOTE_RIGHT_R = "note_right-release";
	var NOTE_DOWN_R = "note_down-release";
	var NOTE_RING_R = "note_ring-release";
	var ACCEPT = "accept";
	var BACK = "back";
	var PAUSE = "pause";
	var RESET = "reset";
...
class Controls extends FlxActionSet
{
	...
	var _note_ring = new FlxActionDigital(Action.NOTE_RING);
	...
	var _note_ringP = new FlxActionDigital(Action.NOTE_RING_P);
	...
	var _note_ringR = new FlxActionDigital(Action.NOTE_RING_R);
	...
	public var NOTE_RING(get, never):Bool;

	inline function get_NOTE_RING()
		return _note_ring.check();
	...
	public var NOTE_RING_P(get, never):Bool;

	inline function get_NOTE_RING_P()
		return _note_ringP.check();
	...
	public var NOTE_RING_R(get, never):Bool;

	inline function get_NOTE_RING_R()
		return _note_ringR.check();
	...
	public function setHitBox(Hitbox:FlxHitbox, Type:FlxHitboxType)
	{
		switch (Type)
		{
			...
			case SPACE:
				...
				inline forEachBound(Control.NOTE_UP, (action, state) -> addButtonNOTES(action, Hitbox.buttonUp, state));
				inline forEachBound(Control.NOTE_DOWN, (action, state) -> addButtonNOTES(action, Hitbox.buttonDown, state));
				inline forEachBound(Control.NOTE_RING, (action, state) -> addButtonNOTES(action, Hitbox.buttonDodge, state));
				inline forEachBound(Control.NOTE_LEFT, (action, state) -> addButtonNOTES(action, Hitbox.buttonLeft, state));
				inline forEachBound(Control.NOTE_RIGHT, (action, state) -> addButtonNOTES(action, Hitbox.buttonRight, state));
		}
	}
	...
	public function setVirtualPadUI(VirtualPad:FlxVirtualPad, DPad:FlxDPadMode, Action:FlxActionMode)
	{
		...
		switch (Action)
		{
			...
			case DODGE:
				inline forEachBound(Control.NOTE_RING, (action, state) -> addButtonUI(action, VirtualPad.buttonD, state));
			case B:
				inline forEachBound(Control.BACK, (action, state) -> addButtonUI(action, VirtualPad.buttonB, state));
			case A_B | A_B_C | A_B_E | A_B_X_Y | A_B_C_D | A_B_C_X_Y | A_B_C_X_Y_Z | A_B_C_D_V_X_Y_Z:
				inline forEachBound(Control.ACCEPT, (action, state) -> addButtonUI(action, VirtualPad.buttonA, state));
				inline forEachBound(Control.BACK, (action, state) -> addButtonUI(action, VirtualPad.buttonB, state));
			case NONE: // do nothing
		}
	...
	public function setVirtualPadNOTES(VirtualPad:FlxVirtualPad, DPad:FlxDPadMode, Action:FlxActionMode)
	{
		...
		switch (Action)
		{
			...
			case DODGE:
				inline forEachBound(Control.NOTE_RING, (action, state) -> addButtonNOTES(action, VirtualPad.buttonD, state));
			case B:
	...
	function forEachBound(control:Control, func:FlxActionDigital->FlxInputState->Void)
	{
		switch (control)
		{
			...
			case NOTE_RING:
				func(_note_ring, PRESSED);
				func(_note_ringP, JUST_PRESSED);
				func(_note_ringR, JUST_RELEASED);
			case ACCEPT:
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			case Solo:
				...
				inline bindKeys(Control.NOTE_RING, keysMap.get('note_ring'));
				...
				inline bindKeys(Control.EMOTE, keysMap.get('emote'));
			case Duo(true):
				...
				inline bindKeys(Control.NOTE_RING, [F]);
				inline bindKeys(Control.ACCEPT, [G, Z]);
				inline bindKeys(Control.BACK, [H, X]);
				inline bindKeys(Control.PAUSE, [ONE]);
				inline bindKeys(Control.RESET, [R]);
				inline bindKeys(Control.EMOTE, [E]);
			case Duo(false):
				...
				inline bindKeys(Control.EMOTE, [T]);
			case None: // nothing
			case Custom: // nothing
		}
		#else
		switch (scheme)
		{
			case Solo:
				...
				bindKeys(Control.NOTE_RING, [SPACE]);
				bindKeys(Control.ACCEPT, [Z, SPACE, ENTER]);
				bindKeys(Control.BACK, [BACKSPACE, ESCAPE]);
				bindKeys(Control.PAUSE, [P, ENTER, ESCAPE]);
				bindKeys(Control.RESET, [R]);
				bindKeys(Control.EMOTE, [E]);
			case Duo(true):
	...
	public function addDefaultGamepad(id):Void
	{
		...
		addGamepadLiteral(id, [
			...
			Control.NOTE_RING => [8, 9, A],
			Control.PAUSE => [LEFT_SHOULDER],
			Control.RESET => [START],
			Control.EMOTE => [RIGHT_SHOULDER]
		]);
	#end
	#if switch
		addGamepadLiteral(id, [
