...
class Note extends FlxSprite
{
	...
	function reloadNote(?sprite:String = '') {
		...
		if(PlayState.isPixelStage) {
			if (sprite != 'fatal-notes' && sprite != 'fatal_new-notes') {
				if(isSustainNote) {
		...
		switch (noteType) {
			case 'Hurt Note', 'Phantom Note', 'Static Note', 'Hex Note', 'Fatal Note', 'Bullet Note', 'Knife Note':
			default:
