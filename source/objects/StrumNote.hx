	...
	public function new(x:Float, y:Float, leData:Int, player:Int, notetype:String) {
		...
		if (PlayState.instance.pixelNoteGenesis) {
			...
			if (PlayState.isPixelStage) {
				...
				if (PlayState.instance.noteChangesFromTheStore)
					skin = 'arrows/'+ClientPrefs.arrowSkin+prefix;
				else
					skin = 'arrows/NOTE_assets'+prefix;
			}
		}
	...
	public function playAnim(anim:String, ?force:Bool = false) {
		...
		if(animation.curAnim == null || animation.curAnim.name == 'static') {
			...
			if (PlayState.instance.customNoteColors) {
				...
				if (PlayState.instance.noteChangesFromTheStore && !PlayState.instance.pixelNoteGenesis && !PlayState.isPixelStage) {
					if (!ClientPrefs.arrowColorsRGB) {
						if (notetype != 'sunky') {
