...
class NoteSplash extends FlxSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, ?note:Int = 0) {
		...
		if (!PlayState.instance.fatalUI && ClientPrefs.fatalSide) {
			if (PlayState.SONG.splashSkin == 'fatal-splash')
				skin = 'fatal-splash';
		}
	...
	public function setupNoteSplash(x:Float, y:Float, note:Int = 0, texture:String = null, hueColor:Float = 0, satColor:Float = 0, brtColor:Float = 0) {
		...
		if (!PlayState.instance.fatalUI && PlayState.SONG.song.toLowerCase() != 'tetrabrachial') {
			...
			if (PlayState.SONG.song.toLowerCase() != 'tetrabrachial') {
				...
				if (PlayState.SONG.song.toLowerCase() == 'tetrabrachial' && ClientPrefs.fatalSide) {
					texture = 'fatal-splash';
				}
			}
		}
		...
		switch(texture){
			...
			default:
				...
				if (skinDetectShit.contains('diamond') || skinDetectShit.contains('electric') || skinDetectShit.contains('sparkles') || skinDetectShit.contains('arrow') || skinDetectShit.contains('x-x') || skinDetectShit.contains('squares')) {
