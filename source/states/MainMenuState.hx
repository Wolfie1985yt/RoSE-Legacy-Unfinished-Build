...
import objects.MainMenuArtsList as MainMenuArtsList;

using StringTools;

class MainMenuState extends MusicBeatState
{
	public static var psychEngineVersion:String = '0.6.3 (v1.0)'; //This is also used for Discord RPC
	...
	#if mobile
	var countOfSongsForMobile:Int = 131;
	#else
	var countOfSongs:Int = FileSystem.readDirectory('assets/songs/').length;
	#end
	...
	var flaFileIsHere:Bool = false;
	var flaFile:FlxSprite;

	var chotixIsHere:Bool = false;
	var chotixQuestion:FlxSprite;
	...
	if (!ClientPrefs.SongsCompleted.contains('test')) {
			#if desktop
			countOfSongs = countOfSongs - 1;
			#else
			countOfSongsForMobile = countOfSongsForMobile - 1;
			#end
		}
		...
		if (ClientPrefs.SongsCompleted.length < #if mobile countOfSongsForMobile #else countOfSongs #end && ClientPrefs.SongsCompleted.length >= 80) {
			compSongsStuffOne = '80';
			compSongsStuffTwo = 0xFFAA5EAF;
			compSongsStuffThree = 0xFF5D3460;
		}
		if (ClientPrefs.SongsCompleted.length == #if mobile countOfSongsForMobile #else countOfSongs #end) {
		...
		var keybinds:FlxText = new FlxText(FlxG.width - 705, FlxG.height - 36, 700, "Press F1 to open debug menu. Press ALT to open keybinds", 3);
		...
		add(songsCompFrame);
		songsCompleted = new FlxText(-558, 30.5, FlxG.width, ClientPrefs.SongsCompleted.length + '/' + #if mobile countOfSongsForMobile #else countOfSongs #end);
		songsCompleted.setFormat(Paths.font("advanced.ttf"), 50, compSongsStuffTwo, CENTER);
		songsCompleted.scrollFactor.set();
		add(songsCompleted);
		
		songsCompletedPercent = (ClientPrefs.SongsCompleted.length / #if mobile countOfSongsForMobile #else countOfSongs #end);
		...
		ShopButton = new FlxSprite(1155,0).loadGraphic(Paths.image('mainmenu/shop'));
	...
	function MenuArtRandom(TotalUnlockedArts:Int, reset:Bool)
	{
		if (!reset) {
			if (TotalUnlockedArts > 1) {
				...
				var RandomChanceOfSecretArt:Int = FlxG.random.int(0, 30);
				if (TotalUnlockedArts > 30) {
					if (RandomChanceOfSecretArt != 15) {
						...
						if (ClientPrefs.lastMenuArt == MainMenuArtsList.secretArts[RandomMenuArt]) {
							...
							if (MainMenuArtsList.secretArts[RandomMenuArt] == 'soink') {
								flaFileIsHere = true;
								flaFile = new FlxSprite(130, 291).loadGraphic(Paths.image('mainmenu/arts/secret/soink-fla'));
								flaFile.antialiasing = ClientPrefs.globalAntialiasing;
								flaFile.scrollFactor.set();
								artsArrayJustForFun.add(flaFile);
							}
							
							if (MainMenuArtsList.secretArts[RandomMenuArt] == 'chotix') {
								chotixIsHere = true;
								chotixQuestion = new FlxSprite(430, -20).loadGraphic(Paths.image('mainmenu/arts/secret/chotix-question'));
								chotixQuestion.antialiasing = ClientPrefs.globalAntialiasing;
								chotixQuestion.scrollFactor.set();
								artsArrayJustForFun.add(chotixQuestion);
							}
						}
					}
				} else {
			...
			flaFileIsHere = false;
			if (flaFile != null) flaFile.kill();
			chotixIsHere = false;
			if (chotixQuestion != null) chotixQuestion.kill();
	...
	override function update(elapsed:Float)
	{
		...
		if (!openTrophyMenu) {
			...
			if (!lockedControl && !selectedSomethin) {
				new FlxTimer().start(0.05, function(tmr:FlxTimer) {
					...
					removeVirtualPad();
					#end
					openSubState(new substates.TrophiesSubstate());
					lockedControl = true;
					selectedSomethin = true;
					lockMouseSelection = true;
				});
		...
		if (FlxG.keys.justPressed.DELETE #if mobile || virtualPad.buttonC.justPressed #end && !lockedControl && !selectedSomethin) {
			...
			removeVirtualPad();
			#end
			openSubState(new substates.EraseProgressSubstate());
			lockedControl = true;
			selectedSomethin = true;
			lockMouseSelection = true;
		}
				...
				if (!FPSMenuInside) {
					if (FlxG.mouse.overlaps(UpdInfoBG)) {
						...
						if (FlxG.mouse.pressed)
							MusicBeatState.switchState(new states.Changelog());
					} else {
						if (versions[versions.length-1].split('<')[1] > Application.current.meta.get('version'))
		...
		if (FlxG.keys.justPressed.R && !lockedControl && !selectedSomethin) {
			flaFileIsHere = false;
			if (flaFile != null) flaFile.kill();
			chotixIsHere = false;
			if (chotixQuestion != null) chotixQuestion.kill();
			if (art != null) art.kill();
			art = new FlxSprite(0, 0).loadGraphic(Paths.image('mainmenu/arts/secret/' + MainMenuArtsList.secretArts[4]));
		
			ClientPrefs.lastMenuArt = MainMenuArtsList.secretArts[4];
			ClientPrefs.saveSettings();
			art.antialiasing = ClientPrefs.globalAntialiasing;
			art.scrollFactor.set();
			art.screenCenter();
			...
			if (MainMenuArtsList.secretArts[4] == 'chotix') {
				chotixIsHere = true;
				chotixQuestion = new FlxSprite(430, -20).loadGraphic(Paths.image('mainmenu/arts/secret/chotix-question'));
				chotixQuestion.antialiasing = ClientPrefs.globalAntialiasing;
				chotixQuestion.scale.x = 0.8;
				chotixQuestion.scale.y = 0.8;
				chotixQuestion.scrollFactor.set();
				artsArrayJustForFun.add(chotixQuestion);
			}
		}
		...
		if (!selectedSomethin && !lockedControl) {
			...
			if (flaFileIsHere) {
				if (FlxG.mouse.overlaps(flaFile) && FlxG.mouse.justReleased) {
					PlayState.SONG = Song.loadFromJson('flashcard-hard', 'flashcard');
					PlayState.storyDifficulty = 2;
					if (!ClientPrefs.charactersUnlocked.contains('soink.fla') && ClientPrefs.gameplaySettings.get("botplay") == false && ClientPrefs.gameplaySettings.get("practice") == false) {
						ClientPrefs.charactersUnlocked.push('soink.fla');
						ClientPrefs.saveSettings();
					}
			...
			if (chotixIsHere) {
				if (FlxG.mouse.overlaps(chotixQuestion) && FlxG.mouse.justReleased) {
					PlayState.SONG = Song.loadFromJson('long-sky-hard', 'long-sky');
					PlayState.storyDifficulty = 2;
					if (!ClientPrefs.charactersUnlocked.contains('chotix') && ClientPrefs.gameplaySettings.get("botplay") == false && ClientPrefs.gameplaySettings.get("practice") == false) {
						ClientPrefs.charactersUnlocked.push('chotix');
						ClientPrefs.saveSettings();
					}
					new FlxTimer().start(0.3, function(tmr:FlxTimer) { 
						Main.InPlaystate = true;
