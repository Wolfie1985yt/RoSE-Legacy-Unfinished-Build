...
import states.NoCheating as NoCheating;
import objects.SonicNumber as SonicNumber;
...
class PlayState extends MusicBeatState
{
	...
	public var healthLock:Bool = false;
	public var healthLockTimer:FlxTimer;
	public var requitalSlash:Bool = false;
	public var requitalSlashTimer:FlxTimer;
	...
	public var prevCombo:Int = 0;
	public var missStack:Int = 0;

	private var healthBarBG:AttachedSprite;
	public var healthBar:FlxBar;
	public var songPercent:Float = 0;
	
	public var curseEnding:Int = 0;
	...
	public var doAutoCamZooms:Bool = true;
	public var camGameZooms:Bool = true;
	...
	public static var videoTime:Int = 0;
	...
	var grpLimoDancers:FlxTypedGroup<objects.BackgroundDancer>;
	...
	var bgGirls:objects.BackgroundGirls;
	...
	public var charWindow:Bool = false;
	...
	public var GameOverCameraMove:Bool = false;
	...
	var requitalSlashThing:FlxSprite;
	...
	public var bfHitbox:objects.Player;
	public var dadHitbox:objects.Player;
	public var eggHitbox:objects.Player;
	public var camHit:objects.Player;

	public var platformsGrp:FlxSpriteGroup;
    public var hitboxesGrp:FlxSpriteGroup;
    var level:Int = 2;
	var levelDad:Int = 2;
	...
	// XTerion
	var xterionFloor:objects.Floor;
	...
	// Sl4sh
	var eyesback:FlxSprite;
	var wallsl4sh:FlxSprite;
	var eyesfront:FlxSprite;
	var pillers:BGSprite;
	var pillersPovF:BGSprite;
	var pillersPovB:BGSprite;
	var floorsl4sh:BGSprite;
	var lavasl4sh:FlxSprite;
	var floorPov:BGSprite;
	var lavaPov:FlxSprite;
	...
	public var SoinkAnimate:FlxSprite;
	public var SoinkAnimateGlitch:FlxSprite;
	
	//New Credits
	var CreditsSplash:FlxSprite;
	var CreditsCodeSplash:FlxSprite;
	var CreditsCodeSplashNicknames:FlxText;
	var CreditsArtSplash:FlxSprite;
	var CreditsArtSplashNicknames:FlxText;
	var CreditsMusicSplash:FlxSprite;
	var CreditsMusicSplashNicknames:FlxText;
	var CreditsChartSplash:FlxSprite;
	var CreditsChartSplashNicknames:FlxText;
	var CreditsEtcSplash:FlxSprite;
	var CreditsEtcSplashMode:FlxText;
	var CreditsAuthorsCode:String = '';
	var CreditsAuthorsArt:String = '';
	var CreditsAuthorsMusic:String = '';
	var CreditsAuthorsChart:String = '';
	var CreditsAuthors:String = '';
	var showCredits:Bool = true;
	
	public var autoChangeAllTextColor:Bool = true;
	...
	function getAlphaBounds(bitmapData:BitmapData):Rectangle {
        ...
        for (y in 0...bitmapData.height) {
            for (x in 0...bitmapData.width) {
                var pixel = bitmapData.getPixel32(x, y); // Get pixel color and alpha
                var alpha = (pixel >> 24) & 0xFF; // Extract the alpha channel
	...
	override public function create()
	{
		...
		switch (PlayState.SONG.song.toLowerCase()) {
			case 'b4cksl4sh' | 'milk' | 'cool-party' | 'prey' | 'prey-good-future':
				#if desktop
		...
		if (SONG == null)
			SONG = Song.loadFromJson('old-ycr-slaps');
		...
		gameovers.GameOverSubstate.resetVariables();
		...
		switch (curStage)
		{
			case 'endless-forest':
				if (SONG.song.toLowerCase() != 'oxxynless') {
					gameovers.GameOverSubstate.characterName = 'endless_bf';
				}
			...
			case 'fun-cd':
				gameovers.GameOverSubstate.characterName = 'BFgreen';
			...
			case 'stage':
				if (SONG.song.toLowerCase()=='test'){
					duo = new FlxSprite(-10100, 0).loadGraphic(Paths.image("bgs/nostalgic-duo/duo"));
					duo.scale.set(0.4, 0.4);
					duo.cameras = [camOther];
					duo.screenCenter(Y);
					add(duo);
				}
			...
			case 'sunkyandsanic':
				if (!ClientPrefs.PotatoOptimization) {	
					cereal = new FlxSprite(-1000, 0).loadGraphic(Paths.image("bgs/milk/cereal"));
					cereal.cameras = [camOther];
					cereal.screenCenter(Y);
					add(cereal);

					munch = new FlxSprite(-1000, 0).loadGraphic(Paths.image("bgs/milk/sunkyMunch"));
					munch.cameras = [camOther];
					munch.screenCenter(Y);
					add(munch);

					pose = new FlxSprite(-1000, 0).loadGraphic(Paths.image("bgs/milk/sunkyPose"));
					pose.cameras = [camOther];
					pose.screenCenter(Y);
					add(pose);

					duo = new FlxSprite(-1000, 0).loadGraphic(Paths.image("bgs/nostalgic-duo/duo"));
					duo.scale.set(0.4, 0.4);
					duo.cameras = [camOther];
					duo.screenCenter(Y);
					add(duo);
			...
			case 'DDDDD':
				gameovers.GameOverSubstate.characterName = 'bf-3d-death';
				gameovers.GameOverSubstate.loopSoundName = 'sunshine-gameover';
			...
			case 'sanicStage':
				gameovers.GameOverSubstate.loopSoundName = '';
				gameovers.GameOverSubstate.endSoundName = '';
				gameovers.GameOverSubstate.characterName = '';
			...
			case 'chamber':
				gameovers.GameOverSubstate.deathSoundName = 'fleetway-death';
				gameovers.GameOverSubstate.loopSoundName = 'fleetway-gameover';
				gameovers.GameOverSubstate.endSoundName = 'fleetway-retry';
			...
			case 'needle':
				...
				gameovers.GameOverSubstate.characterName = 'bf-needle-die';
				gameovers.GameOverSubstate.deathSoundName = 'needlemouse-death';
				gameovers.GameOverSubstate.loopSoundName = 'needlemouse-gameover';
				gameovers.GameOverSubstate.endSoundName = 'needlemouse-retry';
			...
			case 'needle-p2':
				...
				gameovers.GameOverSubstate.characterName = 'bf-needle-die';
				gameovers.GameOverSubstate.deathSoundName = 'needlemouserage-death';
				gameovers.GameOverSubstate.loopSoundName = 'needlemouserage-gameover';
				gameovers.GameOverSubstate.endSoundName = 'needlemouserage-retry';
			...
			case 'needle-encore':
				...
				gameovers.GameOverSubstate.characterName = 'bf-needle-die';
				gameovers.GameOverSubstate.deathSoundName = 'shadowmouse-death';
				gameovers.GameOverSubstate.loopSoundName = 'shadowmouse-gameover';
				gameovers.GameOverSubstate.endSoundName = 'shadowmouse-retry';
			...
			case 'fatality':
				...
				gameovers.GameOverSubstate.characterName = 'bf-fatal-death';
				gameovers.GameOverSubstate.deathSoundName = 'fatal-death';
				gameovers.GameOverSubstate.loopSoundName = 'fatal-gameover';
				gameovers.GameOverSubstate.endSoundName = 'fatal-retry';
			...
			case 'critical-error' | 'fatal-encore':
				...
				gameovers.GameOverSubstate.characterName = 'bf-fatal-death';
				gameovers.GameOverSubstate.deathSoundName = 'encorefatal-death';
				gameovers.GameOverSubstate.loopSoundName = 'fatal-gameover';
				gameovers.GameOverSubstate.endSoundName = 'fatal-retry';

				isPixelStage = true;

				if (ClientPrefs.Shaders && curStage == 'critical-error') {
					tailsShader = new NTSC();
					var filter = new ShaderFilter(tailsShader);
					curShader = filter;
					camHUD.setFilters([curShader]);
				...
				if (curStage == 'fatal-encore') {
					#if windows
					var filePath = "C:\\Users\\" + Main.PCuserName + "\\AppData\\Roaming\\Microsoft\\Windows\\Themes\\TranscodedWallpaper";
					if (sys.FileSystem.exists(filePath)) {
						backend.WinPaths.copyFileFromWinToFNF(filePath, "pcbg.png", true);
					} else {
						trace("Wallpaper file not found! Skipping copy.");
					}
					#end
			...
			case 'soink':
				BackgroundPC = new FlxSprite(-25, -151);
				BackgroundPC.frames = Paths.getSparrowAtlas('bgs/flashcard/bg');
				BackgroundPC.animation.addByPrefix('idle', 'idle', 24);
				BackgroundPC.animation.play('idle');
				BackgroundPC.setGraphicSize(3733, 2100);
				BackgroundPC.scrollFactor.set(0,0);
				BackgroundPC.antialiasing = true;
				add(BackgroundPC);
				
				SoinkAnimate = new FlxSprite(-550, 100);
				SoinkAnimate.frames = Paths.getSparrowAtlas('bgs/flashcard/animate');
				SoinkAnimate.animation.addByIndices('idle', 'idle', [0], "", 24, true);
				SoinkAnimate.animation.addByIndices('glitch', 'idle', [1,2,3,4,5,6], "", 24, true);
				SoinkAnimate.animation.play('idle');
				SoinkAnimate.setGraphicSize(3733, 2100);
				SoinkAnimate.scale.x = 2.3;
				SoinkAnimate.scale.y = 2.3;
				SoinkAnimate.antialiasing = true;
				add(SoinkAnimate);
				
				SoinkAnimateGlitch = new FlxSprite(-550, 100);
				SoinkAnimateGlitch.frames = Paths.getSparrowAtlas('bgs/flashcard/animate');
				SoinkAnimateGlitch.animation.addByIndices('glitch', 'idle', [1,2,3,4,5,6], "", 24, true);
				SoinkAnimateGlitch.animation.play('glitch');
				SoinkAnimateGlitch.setGraphicSize(3733, 2100);
				SoinkAnimateGlitch.scale.x = 2.3;
				SoinkAnimateGlitch.scale.y = 2.3;
				SoinkAnimateGlitch.antialiasing = true;
				add(SoinkAnimateGlitch);
				SoinkAnimateGlitch.alpha = 0;
				
				FatalEncoreTray = new FlxSprite(-1250, 1155).makeGraphic(3733, 117, 0xFF1B1D20);
				FatalEncoreTray.scrollFactor.set(0, 0);
				add(FatalEncoreTray);
				FatalEncoreTray.alpha = 0.95;
				
				FatalEncoreTrayIcons = new FlxSprite(0, 1185).loadGraphic(Paths.image('win10Tray-secret'));
				FatalEncoreTrayIcons.setGraphicSize(3200, 100);
				FatalEncoreTrayIcons.scrollFactor.set(0, 0);
				FatalEncoreTrayIcons.antialiasing = true;
				add(FatalEncoreTrayIcons);
				
				FatalEncoreUsername = new FlxText(450, 1175, FlxG.width, 'Skyeetee');
				FatalEncoreUsername.setFormat(Paths.font('segoeui.ttf'), 40, 0xFFFFFFFF);
				FatalEncoreUsername.alignment = 'right';
				FatalEncoreUsername.scrollFactor.set(0, 0);
				FatalEncoreUsername.updateHitbox();
				...
				FatalEncorePCTime = new FlxText(1385, 1158, FlxG.width, Std.string(DateTools.format(Date.now(), "%T")));
				FatalEncorePCTime.setFormat(Paths.font('segoeui.ttf'), 33, 0xFFFFFFFF);
				FatalEncorePCTime.alignment = 'center';
				FatalEncorePCTime.scrollFactor.set(0, 0);
				FatalEncorePCTime.updateHitbox();
				...
				FatalEncorePCDay = new FlxText(1385, 1201, FlxG.width, Std.string(DateTools.format(Date.now(), "%F")).split("-")[2] + '.' + Std.string(DateTools.format(Date.now(), "%F")).split("-")[1] + '.' + Std.string(DateTools.format(Date.now(), "%F")).split("-")[0]);
				FatalEncorePCDay.setFormat(Paths.font('segoeui.ttf'), 33, 0xFFFFFFFF);
				FatalEncorePCDay.alignment = 'center';
				FatalEncorePCDay.scrollFactor.set(0, 0);
				FatalEncorePCDay.updateHitbox();
			...
			case 'horizon', 'our-horizon':
				gameovers.GameOverSubstate.deathSoundName = 'chaotix-death';
				gameovers.GameOverSubstate.loopSoundName = 'chaotix-gameover';
				gameovers.GameOverSubstate.endSoundName = 'chaotix-retry';
				gameovers.GameOverSubstate.characterName = 'bf-chaotix-death';
			...
			case 'starved-pixel':
				gameovers.GameOverSubstate.characterName = 'bf-sonic-gameover';
				gameovers.GameOverSubstate.deathSoundName = 'prey-death';
				gameovers.GameOverSubstate.loopSoundName = 'prey-gameover';
				gameovers.GameOverSubstate.endSoundName = 'prey-retry';
			...
			case 'starved':
				gameovers.GameOverSubstate.deathSoundName = 'starved-death';
				gameovers.GameOverSubstate.loopSoundName = 'starved-gameover';
				gameovers.GameOverSubstate.endSoundName = 'starved-retry';
				gameovers.GameOverSubstate.characterName = 'bf-starved-die';
				defaultCamZoom = 0.85;
			...
			case 'sl4sh':
				...
				gameovers.GameOverSubstate.deathSoundName = 'slash-death';
				gameovers.GameOverSubstate.loopSoundName = 'slash-gameover';
				gameovers.GameOverSubstate.endSoundName = 'slash-retry';
				gameovers.GameOverSubstate.characterName = 'bf-slash-death';
				
				isPixelStage = true;
				defaultCamZoom = 0.75;
				
				if (!ClientPrefs.PotatoOptimization) {
					eyesback = new FlxSprite(0,0);
					eyesback.frames = Paths.getSparrowAtlas('sl4sh/back eyes',  "exe");
					eyesback.animation.addByPrefix('anim2', 'back eyes idle', 24, true);
					eyesback.animation.play('anim2');
					eyesback.scale.y = 5.5; 
					eyesback.scale.x = 5.5;
					eyesback.antialiasing = false;
					add(eyesback);

					wallsl4sh = new FlxSprite().loadGraphic(Paths.image("sl4sh/wall", "exe"));
					wallsl4sh.scale.y = 5.5; 
					wallsl4sh.scale.x = 5.5;
					wallsl4sh.antialiasing = false;
					add(wallsl4sh);

					eyesfront = new FlxSprite(0, 0);
					eyesfront.frames = Paths.getSparrowAtlas('sl4sh/front eyes', "exe");
					eyesfront.animation.addByPrefix('anim1', 'front eyes idle', 24, true);
					eyesfront.animation.play('anim1');
					eyesfront.scale.y = 5.5; 
					eyesfront.scale.x = 5.5;
					eyesfront.antialiasing = false;
					add(eyesfront);

					pillers = new BGSprite('sl4sh/pillers', "exe", 0, 0, 1, 1);
					pillers.scale.y = 5.5; 
					pillers.scale.x = 5.5;
					pillers.antialiasing = false;
					add(pillers);

					floorsl4sh = new BGSprite('sl4sh/floor', "exe", 0, 0, 1, 1);
					floorsl4sh.scale.y = 5.5; 
					floorsl4sh.scale.x = 5.5;
					floorsl4sh.antialiasing = false;
					add(floorsl4sh);

					lavasl4sh = new FlxSprite(210, 510);
					lavasl4sh.frames = Paths.getSparrowAtlas('sl4sh/slashLava', "exe");
					lavasl4sh.animation.addByPrefix('anim3', 'sl4sh background lava', 24, true);
					lavasl4sh.animation.play('anim3');
					lavasl4sh.scale.y = 12; 
					lavasl4sh.scale.x = 10.3;
					lavasl4sh.scrollFactor.set(0.5, 0.5);
					lavasl4sh.antialiasing = false;
					add(lavasl4sh);

					for (sl4shPart in [eyesback, wallsl4sh, eyesfront, pillers, floorsl4sh])
						sl4shPart.alpha = 0.15;

					//POV Stuff	
					pillersPovF = new BGSprite('sl4sh/pov/povpillarsback', "exe", -250, -180, 1, 1);
					pillersPovF.scale.y = 5.5; 
					pillersPovF.scale.x = 5.5;
					pillersPovF.antialiasing = false;
					pillersPovF.alpha = 0;
					add(pillersPovF);

					lavaPov = new FlxSprite(0, 0);
					lavaPov.frames = Paths.getSparrowAtlas('sl4sh/pov/lavaPOV', "exe");
					lavaPov.animation.addByPrefix('anim4', 'lavaPOV idle', 12, true);
					lavaPov.animation.play('anim4');
					lavaPov.scale.y = 5.5; 
					lavaPov.scale.x = 5.5;
					lavaPov.alpha = 0;
					lavaPov.antialiasing = false;
					add(lavaPov);

					floorPov = new BGSprite('sl4sh/pov/povfloor', "exe", -300, -350, 1, 1);
					floorPov.scale.y = 8.5; 
					floorPov.scale.x = 8.5;
					floorPov.alpha = 0;
					floorPov.antialiasing = false;
					add(floorPov);

					pillersPovB = new BGSprite('sl4sh/pov/povpillars',"exe", -200, -200, 1, 1);
					pillersPovB.scale.y = 5.5; 
					pillersPovB.scale.x = 5.5;
					pillersPovB.antialiasing = false;
					pillersPovB.alpha = 0;
					add(pillersPovB);
				}
			...
			case 'fakerStage':
				fakertransform = new FlxSprite(0, 300);
			...
			case 'lumpy':
				gameovers.GameOverSubstate.deathSoundName = 'lumpy-death';
				gameovers.GameOverSubstate.loopSoundName = 'lumpy-gameover';
				gameovers.GameOverSubstate.endSoundName = 'lumpy-retry';
				gameovers.GameOverSubstate.characterName = 'lumpy-bf-gameover';
		...
		switch (curStage) {
			...
			case 'starved-pixel':
				CoolUtil.precacheSound('thunder1', 'preload');
				CoolUtil.precacheSound('thunder2', 'preload');
				CoolUtil.precacheSound('thunder3', 'preload');
				...
				FlxG.worldBounds.set(0, 0, 100000, 100000);
				bfHitbox = new objects.Player(400,164);
				bfHitbox.solid = true;
				dadHitbox = new objects.Player(200,164);
				dadHitbox.allowJump = false;
				dadHitbox.solid = true;
				eggHitbox = new objects.Player(200,124);
				eggHitbox.solid = false;
				eggHitbox.gravity = 0;
				camHit = new objects.Player(400,64);
				camHit.gravity = 0;
				eggHitbox.solid = false;
				eggHitbox.gravity = 0;
				add(bfHitbox);
				...
				add(boyfriendGroup);
				createLevel();
				generationAlgorithm(true, 1, false, 0);
				add(platformsGrp);
				add(hitboxesGrp);
				hitboxesGrp.solid = true;
				trace(hitboxesGrp.countLiving());
				level = 2;
				if (gpuCache)
		...
		switch(SONG.song.toLowerCase()) {
			...
			case 'endless-us': //Blue
				defaultSplash = true;
				if (!ClientPrefs.arrowColorsRGB) customNoteColors = true;
				customArrowColors = [[-161, 9, -20], [-161, 9, -20], [-161, 9, -20], [-161, 9, -20]];
			case 'endless-jp': //Purple
				defaultSplash = true;
				if (!ClientPrefs.arrowColorsRGB) customNoteColors = true;
				customArrowColors = [[-155, -5, 30], [-155, -5, 30], [-155, -5, 30], [-155, -5, 30]];
			case 'chaos', 'chaos-encore', 'terror': //Super
				defaultSplash = true;
				if (!ClientPrefs.arrowColorsRGB) customNoteColors = true;
			...
			case 'animosity', 'reunion', 'ugly', 'hedge', 'her-world', 'her-world-encore', 'her-world-old', 'uh-oh', 'lukas-world', 'malediction', 'extricate-hex', 'miasma', 'personel', 'personel-serious', 'relax', 'soured', 'substantial', 'thunderbolt', 'shocker', 'extreme-zap': //Default Splashes
				defaultSplash = true;
		}
			...
			switch (SONG.song.toLowerCase()) {
				...
				case 'agony':
					if (!ClientPrefs.charactersUnlocked.contains('sally alt') && chartingMode)
						MusicBeatState.switchState(new NoCheating());
				case 'bloodstorm' | 'sharpy-showdown':
				...
				case 'fatality' | 'critical-error':
					if (!ClientPrefs.charactersUnlocked.contains('fatal error') && chartingMode)
						MusicBeatState.switchState(new NoCheating());
				case 'prey' | 'prey-good-future' | 'fight-or-flight':
				...
				case 'burning':
					if (!ClientPrefs.charactersUnlocked.contains('shp and genesys') && chartingMode)
						MusicBeatState.switchState(new NoCheating());
				case 'trickery' | 'perdition' | 'infernal-demise':
				...
				//Secret
				case 'flashcard':
					if (!ClientPrefs.SongsCompleted.contains('flashcard') && chartingMode)
						MusicBeatState.switchState(new NoCheating());
			}
		}
		...
		//Preload Shit
		if (SONG.needsVoices) {
			CoolUtil.precacheVoices('Voices');
		}
		CoolUtil.precacheInst('Inst');
		...
		preload('pauseStuff/frame', 'shared');
		preload('pauseStuff/topbar', 'shared');
		...
		if(SONG.song.toLowerCase() == 'malediction') {
			preload('bgs/curse/malediction/Sky');
			preload('bgs/curse/malediction/treesback');
			preload('bgs/curse/malediction/Treesfront');
			preload('bgs/curse/malediction/Floorback');
			preload('bgs/curse/malediction/Floorfront');
			preload('bgs/curse/malediction/Male addiction Fountain');
			preload('bgs/curse/good/sky');
			preload('bgs/curse/good/light_back');
			preload('bgs/curse/good/light_front');
			preload('bgs/curse/good/day/clouds2');
			preload('bgs/curse/good/day/clouds1');
			preload('bgs/curse/good/day/Curse_water_Day');
			preload('bgs/curse/good/day/beach');
			preload('bgs/curse/good/day/bridge');
			preload('bgs/curse/bad/sky');
			preload('bgs/curse/bad/city2');
			preload('bgs/curse/bad/city1');
			preload('bgs/curse/bad/trees');
			preload('bgs/curse/bad/floor');
			preload('bgs/curse/malediction/rockslide/sky');
			preload('bgs/curse/malediction/rockslide/waybackmoutains');
			preload('bgs/curse/malediction/rockslide/backclouds');
			preload('bgs/curse/malediction/rockslide/midmoutains');
			preload('bgs/curse/malediction/rockslide/frontclouds');
			preload('bgs/curse/malediction/rockslide/siderocks');
			preload('bgs/curse/malediction/rockslide/backrocks');
			preload('bgs/curse/malediction/rockslide/ground');
			preload('bgs/curse/malediction/rockslide/frontrocks');
		}
		...
		preload('credits/art', 'shared');
		preload('credits/chart', 'shared');
		preload('credits/music', 'shared');
		preload('credits/code', 'shared');
		preload('credits/etc', 'shared');
		preload('credits/credits', 'shared');
		preload('language-translation/ru/credits/credits');
		preload('language-translation/es/credits/credits');
		...
		if (discordSongPic && !isSound) {
			...
			switch (SONG.song.toLowerCase())
			{
				...
				case 'bloodstorm' | 'sharpy-showdown': DiscordClient.imageRpc = 'exetior';
				case 'chaos': DiscordClient.imageRpc = 'fleetway';
				case 'fatality': DiscordClient.imageRpc = 'fatal-error';
				case 'critical-error': DiscordClient.imageRpc = 'critical-error';
				case 'prey' | 'prey-good-future': DiscordClient.imageRpc = 'furnace';
				...
				case 'uh-oh': DiscordClient.imageRpc = 'uh-oh';
				...
				case 'trickery' | 'perdition' | 'infernal-demise': DiscordClient.imageRpc = 'satanos';
				...
				//Extras
				case 'her-world-old' | 'long-sky' | 'too-fest-old' | 'tetrabrachial' | 'my-horizon-wechidna' | 'perdition-apollyon' | 'gotta-go-red-glove' | 'hollow-old' | 'ice-cap' | 'haze-og' | 'cool-party' | 'call-of-justice-old' | 'hill-of-the-void' | 'execution-inski' | 'melting-old' | 'color-blind-old' | 'playful' | 'drippyshine' | 'lukas-world' | 'terror' | 'unresponsive' | 'endeavors-og' | 'nostalgic-duo' | 'thunderbolt' | 'test': DiscordClient.imageRpc = 'extras-songs';
				
				//Secret
				case 'flashcard': DiscordClient.imageRpc = 'soink_fla';
			}
		}
		if (discordSongPic && isSound) {
			...
			if (!chartingMode) {
				switch (SONG.song.toLowerCase())
				{
					...
					case 'bloodstorm' | 'sharpy-showdown': DiscordClient.smallImageRpc = 'exetior';
					case 'chaos': DiscordClient.smallImageRpc = 'fleetway';
					case 'fatality': DiscordClient.smallImageRpc = 'fatal-error';
					case 'critical-error': DiscordClient.smallImageRpc = 'critical-error';
					case 'prey' | 'prey-good-future': DiscordClient.smallImageRpc = 'furnace';
					...
					case 'uh-oh': DiscordClient.smallImageRpc = 'uh-oh';
					...
					case 'trickery' | 'perdition' | 'infernal-demise': DiscordClient.smallImageRpc = 'satanos';
		...
		if (SONG.song.toLowerCase() == 'execution' || SONG.song.toLowerCase() == 'cycles' || SONG.song.toLowerCase() == 'hellbent' || SONG.song.toLowerCase() == 'fate') {
			gameovers.GameOverSubstate.loopSoundName = 'lordx-gameover';
			gameovers.GameOverSubstate.endSoundName = 'lordx-retry';
			gameovers.GameOverSubstate.characterName = 'LordXGameOver';
		}
		
		if (SONG.song.toLowerCase() == 'cycles-encore') {
			gameovers.GameOverSubstate.loopSoundName = 'encorelordx-gameover';
			gameovers.GameOverSubstate.endSoundName = 'lordx-retry';
			gameovers.GameOverSubstate.characterName = 'cycles-encore-death';
		}
		...
		if (SONG.song.toLowerCase() == 'too-slow' || SONG.song.toLowerCase() == 'you-cant-run' || SONG.song.toLowerCase() == 'triple-trouble' || SONG.song.toLowerCase() == 'final-escape' || SONG.song.toLowerCase() == 'hill-of-the-void') {
			gameovers.GameOverSubstate.characterName = 'bf-exe-death';
			preload('sonicJUMPSCARE', 'exe');
		}
		
		if (SONG.song.toLowerCase() == 'too-slow' || SONG.song.toLowerCase() == 'you-cant-run' || SONG.song.toLowerCase() == 'melting' || SONG.song.toLowerCase() == 'melting-old' || SONG.song.toLowerCase() == 'triple-trouble' || SONG.song.toLowerCase() == 'faker' || SONG.song.toLowerCase() == 'b4cksl4sh' || SONG.song.toLowerCase() == 'fake-baby' || SONG.song.toLowerCase() == 'my-horizon-wechidna') {
			preload('daSTAT', 'exe');
		}
		
		if (SONG.song.toLowerCase() == 'you-cant-run-encore' || SONG.song.toLowerCase() == 'missiletoe' || SONG.song.toLowerCase() == 'melting') {
			preload('Phase3Static', 'exe');
		}
		
		if (SONG.song.toLowerCase() == 'melting' || SONG.song.toLowerCase() == 'melting-old'){
			preload('JUMPSCARES/Melthog', 'exe');
		}
		
		if (SONG.song.toLowerCase() == 'tetrabrachial') {
			preload('error_popups', 'exe');
			preload('bgs/melt/classic/loop');
			preload('bgs/melt/classic/main');
			preload('bgs/melt/classic/Melthog_S1_stage');
			preload('bgs/melt/classic/plate');
			preload('bgs/melt/classic/water');
			preload('bgs/melt/alt/loop_red');
			preload('bgs/melt/alt/main_red');
			preload('bgs/melt/alt/Melthog_S1_stage_red');
			preload('bgs/melt/alt/plate_red');
			preload('bgs/melt/alt/water_red');
			preload('characters/SL4SH/frontviewsl4sh');
			preload('characters/FatalAndCritical/fatal_true_playable');
			preload('bgs/fatal-encore/Error_Screen');
			preload('bgs/sally-bg/pixel/sonic_alt');
			preload('bgs/sally-bg/pixel/pinball');
			preload('bgs/sally-bg/pixel/floor');
			preload('bgs/sl4shandfatal/preyBG');
			preload('bgs/prey/stardustFloor');
			preload('chaotix/corrupt_background', 'exe');
			preload('chaotix/horizonFuckedUp', 'exe');
			preload('bgs/melt/tv-icons');
			preload('chaotix/horizonsky', 'exe');
			preload('chaotix/horizonFg', 'exe');
			preload('bgs/sally-bg/pixel/floor');
			preload('bgs/sally-bg/pixel/pinball');
			preload('bgs/sally-bg/pixel/sonic_alt');
			preload('bgs/sl4shandfatal/fatalPOV');
			preload('bgs/sl4shandfatal/line');
			preload('bgs/sl4shandfatal/slashPOV');
			preload('fatal/domain', 'exe');
			preload('fatal/domain2', 'exe');
			preload('chaotix/horizonFg', 'exe');
			preload('chaotix/horizonsky', 'exe');
		...
		if (SONG.song.toLowerCase() == 'too-slow-encore' || SONG.song.toLowerCase() == 'too-slow-encore-alt') {	
			preload('characters/BF/HD/ENCORE_BF');
			preload('epicjump', 'exe');
			preload('grimbojump', 'exe');
		}
		...
		if (SONG.song.toLowerCase() == 'final-escape') {
			...
			preload('characters/BF/Pixel/BF');
			preload('characters/XenoAndGrimbo/Pixel/xeno-fake-fe-pix');
			preload('slash-splash', 'shared');
			if (noteChangesFromTheStore)
				preload('arrows/'+ClientPrefs.arrowSkin, 'shared');
			else
		...
		if (SONG.song.toLowerCase() == 'you-cant-run-encore') {
			...
			preloadVideo('the-gaze-of-a-god_NoAudio');
			if (noteChangesFromTheStore)
				preload('arrows/'+ClientPrefs.arrowSkin, 'shared');
			else
				preload('arrows/NOTE_assets', 'shared');
		...
		if (SONG.song.toLowerCase() == 'too-fest' || SONG.song.toLowerCase() == 'nostalgic-duo') {
			preloadVideo('explosion');
			preloadVideo('glasses');
			preloadVideo('guns');
			preloadVideo('hitmarkers');
			preloadVideo('Illuminati');
			preloadVideo('IlluminatiConfirmed');
			preloadVideo('mlg');
			preloadVideo('noscope');
			preloadVideo('weed');
		}
		
		if (SONG.song.toLowerCase() == 'triple-trouble') {
			preload('JUMPSCARES/Eggman', 'exe');
			preload('JUMPSCARES/Knuckles', 'exe');
			preload('JUMPSCARES/Tails', 'exe');
			preload('Phase3Static', 'exe');
		}
		...
		if (curStage == 'christmas') {
			gameovers.GameOverSubstate.deathSoundName = 'christmas-death';
			gameovers.GameOverSubstate.loopSoundName = 'christmas-gameover';
			gameovers.GameOverSubstate.endSoundName = 'christmas-retry';
			gameovers.GameOverSubstate.characterName = 'sonicexe_christmas';
			doAutoCamZooms = false;
		}
		
		if (curStage == 'schoolhouse') {
			gameovers.GameOverSubstate.deathSoundName = 'educator-death';
			gameovers.GameOverSubstate.loopSoundName = 'educator-gameover';
			gameovers.GameOverSubstate.endSoundName = 'educator-retry';
			gameovers.GameOverSubstate.characterName = 'educator-death';
		}
		...
		if (SONG.song.toLowerCase() == 'endless' || SONG.song.toLowerCase() == 'endless-og' || SONG.song.toLowerCase() == 'endless-us' || SONG.song.toLowerCase() == 'endless-jp' || SONG.song.toLowerCase() == 'endeavors' || SONG.song.toLowerCase() == 'endeavors-og') {
				gameovers.GameOverSubstate.deathSoundName = 'majin-death';
				gameovers.GameOverSubstate.loopSoundName = 'majin-gameover';
				gameovers.GameOverSubstate.endSoundName = 'majin-retry';
				gameovers.GameOverSubstate.characterName = 'endless_bf';
		}
		
		if (SONG.song.toLowerCase() == 'endless-encore') {
				gameovers.GameOverSubstate.deathSoundName = 'encoremajin-death';
				gameovers.GameOverSubstate.loopSoundName = 'encoremajin-gameover';
				gameovers.GameOverSubstate.endSoundName = 'majin-retry';
		}
		
		if (SONG.song.toLowerCase() == 'face-off' || SONG.song.toLowerCase() == 'ice-cap') {
				gameovers.GameOverSubstate.deathSoundName = 'lookalike-death';
				gameovers.GameOverSubstate.loopSoundName = 'lookalike-gameover';
				gameovers.GameOverSubstate.endSoundName = 'lookalike-retry';
		}
		
		if (SONG.song.toLowerCase() == 'milk' || SONG.song.toLowerCase() == 'cool-party') {
				gameovers.GameOverSubstate.deathSoundName = 'sunky-death';
				gameovers.GameOverSubstate.loopSoundName = 'sunky-gameover';
				gameovers.GameOverSubstate.endSoundName = 'sunky-retry';
		}
		
		if (SONG.song.toLowerCase() == 'sunshine') {
				gameovers.GameOverSubstate.deathSoundName = 'sunshine-death';
				gameovers.GameOverSubstate.loopSoundName = 'sunshine-gameover';
				gameovers.GameOverSubstate.endSoundName = 'sunshine-retry';
		}
		
		if (SONG.song.toLowerCase() == 'soulless') {
				gameovers.GameOverSubstate.deathSoundName = 'td-death';
				gameovers.GameOverSubstate.loopSoundName = 'td-gameover';
				gameovers.GameOverSubstate.endSoundName = 'td-retry';
		}
		
		if (SONG.song.toLowerCase() == 'faker') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'faker-gameover';
				gameovers.GameOverSubstate.endSoundName = 'faker-retry';
		
		}
		
		if (SONG.song.toLowerCase() == 'her-world' || SONG.song.toLowerCase() == 'her-world-old') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'luther-gameover';
				gameovers.GameOverSubstate.endSoundName = 'luther-retry';
		}
		
		if (SONG.song.toLowerCase() == 'her-world-encore') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'sunter-gameover';
				gameovers.GameOverSubstate.endSoundName = 'sunter-retry';
		}
		
		if (SONG.song.toLowerCase() == 'lukas-world') {
				gameovers.GameOverSubstate.deathSoundName = 'lukas-death';
				gameovers.GameOverSubstate.loopSoundName = 'lukas-gameover';
				gameovers.GameOverSubstate.endSoundName = 'lukas-retry';
		}
		
		if (SONG.song.toLowerCase() == 'mania') {
				gameovers.GameOverSubstate.deathSoundName = 'shtails-death';
				gameovers.GameOverSubstate.loopSoundName = 'shtails-gameover';
				gameovers.GameOverSubstate.endSoundName = 'shtails-retry';
		}
		
		if (SONG.song.toLowerCase() == 'uh-oh') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'demogri-gameover';
				gameovers.GameOverSubstate.endSoundName = 'demogri-retry';
		}
		
		if (SONG.song.toLowerCase() == 'digitalized' || SONG.song.toLowerCase() == 'substantial') {
				gameovers.GameOverSubstate.deathSoundName = 'xterion-death';
				gameovers.GameOverSubstate.loopSoundName = 'xterion-gameover';
				gameovers.GameOverSubstate.endSoundName = 'xterion-retry';
		}
		
		if (SONG.song.toLowerCase() == 'burning' || SONG.song.toLowerCase() == 'burning-og') {
				gameovers.GameOverSubstate.deathSoundName = 'burning-death';
				gameovers.GameOverSubstate.loopSoundName = 'burning-gameover';
				gameovers.GameOverSubstate.endSoundName = 'burning-retry';
		}
		
		if (SONG.song.toLowerCase() == 'long-sky') {
				gameovers.GameOverSubstate.deathSoundName = 'chotix-death';
				gameovers.GameOverSubstate.loopSoundName = 'chotix-gameover';
				gameovers.GameOverSubstate.endSoundName = 'chotix-retry';
		}
		
		if (SONG.song.toLowerCase() == 'hedge') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'hog-gameover';
				gameovers.GameOverSubstate.endSoundName = 'hog-retry';
		}
		
		if (SONG.song.toLowerCase() == 'manual-blast') {
				gameovers.GameOverSubstate.deathSoundName = 'scorched-death';
				gameovers.GameOverSubstate.loopSoundName = 'scorched-gameover';
				gameovers.GameOverSubstate.endSoundName = 'scorched-retry';
		}
		
		if (SONG.song.toLowerCase() == 'malediction' || SONG.song.toLowerCase() == 'extricate-hex' || SONG.song.toLowerCase() == 'unblessful-hedgehog') {
				gameovers.GameOverSubstate.deathSoundName = 'curse-death';
				gameovers.GameOverSubstate.loopSoundName = 'curse-gameover';
				gameovers.GameOverSubstate.endSoundName = 'curse-retry';
		}
		
		if (SONG.song.toLowerCase() == 'found-you') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'normalcd-gameover';
				gameovers.GameOverSubstate.endSoundName = 'normalcd-retry';
		}
		
		if (SONG.song.toLowerCase() == 'genesis' || SONG.song.toLowerCase() == 'corinthians' || SONG.song.toLowerCase() == 'perdition-apollyon') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'gameOver';
				gameovers.GameOverSubstate.endSoundName = 'apollyon-retry';
		}
		
		if (SONG.song.toLowerCase() == 'miasma') {
				gameovers.GameOverSubstate.deathSoundName = 'dsk-death';
				gameovers.GameOverSubstate.loopSoundName = 'dsk-gameover';
				gameovers.GameOverSubstate.endSoundName = 'dsk-retry';
		}
		
		if (SONG.song.toLowerCase() == 'color-blind' || SONG.song.toLowerCase() == 'color-blind-old' || SONG.song.toLowerCase() == 'color-blind-encore') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'mono-gameover';
				gameovers.GameOverSubstate.endSoundName = 'mono-retry';
		}
		
		if (SONG.song.toLowerCase() == 'universal-collapse' || SONG.song.toLowerCase() == 'planestrider' || SONG.song.toLowerCase() == 'planestrider-annie-take') {
				gameovers.GameOverSubstate.deathSoundName = 'omw-death';
				gameovers.GameOverSubstate.loopSoundName = 'omw-gameover';
				gameovers.GameOverSubstate.endSoundName = 'omw-retry';
		}
		
		if (SONG.song.toLowerCase() == 'life-and-death' || SONG.song.toLowerCase() == 'gods-will') {
				gameovers.GameOverSubstate.deathSoundName = 'bratwurst-death';
				gameovers.GameOverSubstate.loopSoundName = 'bratwurst-gameover';
				gameovers.GameOverSubstate.endSoundName = 'bratwurst-retry';
		}
		
		if (SONG.song.toLowerCase() == 'fake-baby') {
				gameovers.GameOverSubstate.deathSoundName = 'nmi-death';
				gameovers.GameOverSubstate.loopSoundName = 'nmi-gameover';
				gameovers.GameOverSubstate.endSoundName = 'nmi-retry';
		}
		
		if (SONG.song.toLowerCase() == 'envy') {
				gameovers.GameOverSubstate.deathSoundName = 'grimeware-death';
				gameovers.GameOverSubstate.loopSoundName = 'grimeware-gameover';
				gameovers.GameOverSubstate.endSoundName = 'grimeware-retry';
		}
		
		if (SONG.song.toLowerCase() == 'bloodstorm' || SONG.song.toLowerCase() == 'sharpy-showdown') {
				gameovers.GameOverSubstate.deathSoundName = 'exetior-death';
				gameovers.GameOverSubstate.loopSoundName = 'exetior-gameover';
				gameovers.GameOverSubstate.endSoundName = 'exetior-retry';
		}
		
		if (SONG.song.toLowerCase() == 'call-of-justice' || SONG.song.toLowerCase() == 'gotta-go' || SONG.song.toLowerCase() == 'call-of-justice-old') {
				gameovers.GameOverSubstate.deathSoundName = 'batman-death';
				gameovers.GameOverSubstate.loopSoundName = 'batman-gameover';
				gameovers.GameOverSubstate.endSoundName = 'batman-retry';
		}
		
		if (SONG.song.toLowerCase() == 'shocker' || SONG.song.toLowerCase() == 'extreme-zap' || SONG.song.toLowerCase() == 'thunderbolt') {
				gameovers.GameOverSubstate.deathSoundName = 'sonichu-death';
				gameovers.GameOverSubstate.loopSoundName = 'sonichu-gameover';
				gameovers.GameOverSubstate.endSoundName = 'sonichu-retry';
				gameovers.GameOverSubstate.characterName = 'bf-sonichu-death';
		}
		
		if (SONG.song.toLowerCase() == 'ugly') {
				gameovers.GameOverSubstate.deathSoundName = 'ugly-death';
				gameovers.GameOverSubstate.loopSoundName = 'ugly-gameover';
				gameovers.GameOverSubstate.endSoundName = 'ugly-retry';
		}
		
		if (SONG.song.toLowerCase() == 'soured') {
				gameovers.GameOverSubstate.deathSoundName = 'lemon-death';
				gameovers.GameOverSubstate.loopSoundName = 'lemon-gameover';
				gameovers.GameOverSubstate.endSoundName = 'lemon-retry';
		}
		
		if (SONG.song.toLowerCase() == 'old-ycr-slaps') {
				gameovers.GameOverSubstate.deathSoundName = 'xenoslaps-death';
				gameovers.GameOverSubstate.loopSoundName = 'xenoslaps-gameover';
				gameovers.GameOverSubstate.endSoundName = 'xenoslaps-retry';
				gameovers.GameOverSubstate.characterName = 'bf-exe-death';
		}
		
		if (SONG.song.toLowerCase() == 'oxxynless') {
				gameovers.GameOverSubstate.deathSoundName = 'oxxy-death';
				gameovers.GameOverSubstate.loopSoundName = 'oxxy-gameover';
				gameovers.GameOverSubstate.endSoundName = 'oxxy-retry'+FlxG.random.int(1, 3);
		}
		
		if (SONG.song.toLowerCase() == 'execution-inski') {
				gameovers.GameOverSubstate.deathSoundName = 'inski-death';
				gameovers.GameOverSubstate.loopSoundName = 'inski-gameover';
				gameovers.GameOverSubstate.endSoundName = 'inski-retry';
				gameovers.GameOverSubstate.characterName = 'LordXGameOver';
		}
		
		if (SONG.song.toLowerCase() == 'flashcard') {
				gameovers.GameOverSubstate.deathSoundName = 'soink-death';
				gameovers.GameOverSubstate.loopSoundName = 'soink-gameover';
				gameovers.GameOverSubstate.endSoundName = 'soink-retry';
		}
		
		if (SONG.song.toLowerCase() == 'trickery' || SONG.song.toLowerCase() == 'perdition' || SONG.song.toLowerCase() == 'infernal-demise') {
				gameovers.GameOverSubstate.deathSoundName = 'satanos-death';
				gameovers.GameOverSubstate.loopSoundName = 'satanos-gameover';
				gameovers.GameOverSubstate.endSoundName = 'satanos-retry';
		}
		
		if (SONG.song.toLowerCase() == 'hollow' || SONG.song.toLowerCase() == 'hollow-old' || SONG.song.toLowerCase() == 'empty' || SONG.song.toLowerCase() == 'hollow-encore') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'devoid-gameover';
				gameovers.GameOverSubstate.endSoundName = 'devoid-retry';
		}
		
		if (SONG.song.toLowerCase() == 'forever-unnamed') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'noname-gameover';
				gameovers.GameOverSubstate.endSoundName = 'gameOverEnd';
		}
		
		if (SONG.song.toLowerCase() == 'coulrophobia') {
				gameovers.GameOverSubstate.deathSoundName = 'rosy_alt-death';
				gameovers.GameOverSubstate.loopSoundName = 'rosy_alt-gameover';
				gameovers.GameOverSubstate.endSoundName = 'rosy_alt-retry';
		}
		
		if (SONG.song.toLowerCase() == 'tribal') {
				gameovers.GameOverSubstate.deathSoundName = 'knuckles_alt-death';
				gameovers.GameOverSubstate.loopSoundName = 'knuckles_alt-gameover';
				gameovers.GameOverSubstate.endSoundName = 'knuckles_alt-retry';
		}
		
		if (SONG.song.toLowerCase() == 'gotta-go-red-glove') {
				gameovers.GameOverSubstate.loopSoundName = 'redglove-gameover';
				gameovers.GameOverSubstate.endSoundName = 'lordx-retry';
				gameovers.GameOverSubstate.characterName = 'LordXGameOver';
		}
		
		if (SONG.song.toLowerCase() == 'broken-heart') {
				gameovers.GameOverSubstate.deathSoundName = 'amy_alt-death';
				gameovers.GameOverSubstate.loopSoundName = 'amy_alt-gameover';
				gameovers.GameOverSubstate.endSoundName = 'amy_alt-retry';
				GameOverCameraMove = false;
		}
		
		if (SONG.song.toLowerCase() == 'goddess') {
				gameovers.GameOverSubstate.deathSoundName = 'fnf_loss_sfx';
				gameovers.GameOverSubstate.loopSoundName = 'sally_alt-gameover';
				gameovers.GameOverSubstate.endSoundName = 'gameOverEnd';
				GameOverCameraMove = false;
		}
		
		if (SONG.song.toLowerCase() == 'too-far' || SONG.song.toLowerCase() == 'too-far-alt' || SONG.song.toLowerCase() == 'last-hope') {
				gameovers.GameOverSubstate.loopSoundName = 'gameover-gameover';
				gameovers.GameOverSubstate.endSoundName = 'gameover-retry';
		}
		
		if (SONG.song.toLowerCase() == 'drippyshine') {
				gameovers.GameOverSubstate.deathSoundName = 'drippy-death';
				gameovers.GameOverSubstate.loopSoundName = 'drippy-gameover';
				gameovers.GameOverSubstate.endSoundName = 'drippy-retry';
		}
		...
		switch (SONG.song.toLowerCase())
		{
			case 'trickery' | 'perdition' | 'perdition-apollyon' |'infernal-demise' | 'corinthians' | 'mania' | 'coulrophobia' | 'flashcard':
				gfGroup.visible = false;
		}
		...
		#if MODS_ALLOWED
		foldersToCheck.insert(0, Paths.mods('globalScripts/'));
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
		...
		#end
		#if mobile
		var doPushMore:Bool = false;
		var luaScript = Paths.getPreloadPath('globalScripts/' + 'LRatio' + '.lua');
		var luaScript2 = Paths.getPreloadPath('globalScripts/' + 'Note&IconFixes' + '.lua');
		var luaScript3 = Paths.getPreloadPath('globalScripts/' + 'scorefix' + '.lua');
		if (OpenFlAssets.exists(luaScript) || OpenFlAssets.exists(luaScript2))
		{
			doPushMore = true;
		}
					
		if(doPushMore) 
			luaArray.push(new FunkinLua(backend.Asset2File.getPath(luaScript)));
			luaArray.push(new FunkinLua(backend.Asset2File.getPath(luaScript2)));
			luaArray.push(new FunkinLua(backend.Asset2File.getPath(luaScript3)));
		#end
		#end
		...
		if (!stageData.hide_girlfriend)
		{
			...
			*/
			if (isFreeplay || isEncore || isExtras || (isSound && isStoryMode)) {
				switch (SONG.song.toLowerCase())
				{
					case 'too-slow-encore', 'too-slow-encore-alt', 'you-cant-run-encore', 'my-horizon-wechidna':
						GFEncoreSkins = false;
						GFSkins = true;
		...
		*/
		if (isFreeplay || isEncore || isExtras || (isSound && isStoryMode)) {
			switch (SONG.song.toLowerCase())
			{
				case 'gotta-go-red-glove', 'hollow-encore', 'hollow', 'hollow-old', 'empty', 'execution', 'cycles', 'fate', 'broken-heart', 'too-fest', 'trickery', 'perdition', 'perdition-apollyon', 'infernal-demise', 'forever-unnamed', 'sharpy-showdown', 'personel-serious', 'ugly', 'hedge', 'manual-blast', 'miasma', 'my-horizon-wechidna':
					BFEncoreSkins = false;
					BFSkins = true;
		...
		switch (curStage)
		{
			...
			case 'sl4sh':
				...
				//cool sl4sh dark shit
				dad.color = 0xFF414141;
				boyfriend.color = 0xFF414141;
		...
		if (OpenFlAssets.exists(file)) {
			dialogueJson = objects.DialogueBoxPsych.parseDialogue(file);
		}
		...
		var doof:objects.DialogueBox = new objects.DialogueBox(false, dialogue);
		// doof.x += 70;
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'sharpy-showdown' | 'bloodstorm':
				timeBar.createFilledBar(0x00FF0000, 0xFF062D68);
				TimeBarColorOne = 0x00FF0000;
				TimeBarColorTwo = 0xFF062D68;
			case 'test':
				timeBar.createFilledBar(0x0000a8f3, 0xFF00a8f3);
				TimeBarColorOne = 0x0000a8f3;
				TimeBarColorTwo = 0x0000a8f3;
			case 'found-you':
			...
			case 'flashcard':
				timeBar.createFilledBar(0x00FF0000, 0xFF0001CB);
				TimeBarColorOne = 0x00FF0000;
				TimeBarColorTwo = 0xFF0001CB;
		}
		...
		if (SONG.song.toLowerCase()=='prey' || SONG.song.toLowerCase()=='prey-good-future') {
			// FlxG.fullscreen = false;
			// FlxG.resizeWindow(1280, 896);
			// FlxG.scaleMode = new RatioScaleMode(true);
			// var window = lime.app.Application.current.window;
			// window.fullscreen = false;
			// window.resizable = false;
			// window.x = Math.floor((Capabilities.screenResolutionX / 2) - (1280 / 2));
			// window.y = Math.floor((Capabilities.screenResolutionY / 2) - (896 / 2));
		...
		if (!xenoEncUI) {
			...
			if (dad.curCharacter == 'soink') AnimatedDadIcon = true;
		...
		#if !mobile
		if(SONG.song.toLowerCase() == 'universal-collapse') {			
			skipTxt = new FlxText(0,625, FlxG.width, "", 40);
			skipTxt.setFormat(Paths.font("vcr.ttf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		for(i in 0...4){
			...
			if (ClientPrefs.Language == 'Spanish' && hudFolder == 'sonic1') number.x += 34;
		...
		#if mobile
		switch (SONG.song.toLowerCase()) {
			case 'chaos' | 'chaos-encore' | 'b4cksl4sh' | 'coulrophobia' | 'hedge':
		...
		#if LUA_ALLOWED
		var filesPushed:Array<String> = [];
		var foldersToCheck:Array<String> = [Paths.getPreloadPath('data/' + Paths.formatToSongPath(SONG.song) + '/scripts/')];

		#if MODS_ALLOWED
		foldersToCheck.insert(0, Paths.mods('data/' + Paths.formatToSongPath(SONG.song) + '/scripts/'));
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
			foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/data/' + Paths.formatToSongPath(SONG.song) + '/scripts/'));
		#end
		...
		var daSong:String = Paths.formatToSongPath(curSong);
			switch (daSong)
			{
				case 'soulless' | 'final-escape' | 'personel' | 'personel-serious' | 'flashcard':
					if (daSong == 'soulless')
						camHUD.alpha = 0;
				...
				case 'endless' | 'endless-jp' | 'endless-og' | 'chaos-encore' | 'drippyshine' | 'endless-encore' | 'envy' | 'gorefest' | 'lurk' | 'skincrawler' | 'faker-encore' | 'forever-unnamed' | 'found-you' | 'frenzy' | 'haze' | 'haze-og' | 'her-world-encore' | 'hollow-encore' | 'uh-oh' | 'color-blind-old' | 'last-hope' | 'long-sky' | 'lukas-world' | 'marauder' | 'melting-old' | 'miasma' | 'oxxynless' | 'perdition-apollyon' | 'personel-serious' | 'personel' | 'planestrider' | 'reunion' | 'round-a-bout-encore' | 'sharpy-showdown' | 'soured' | 'sunshine-encore' | 'sunshine' | 'terror' | 'too-fest' | 'ugly' | 'unblessful-hedgehog' | 'old-ycr-slaps' | 'endless-us' | 'prey' | 'prey-good-future' | 'call-of-justice-old' | 'fight-or-flight'| 'round-a-bout' | 'b4cksl4sh' | 'face-off' | 'ice-cap' | 'malediction' | 'faker' | 'black-sun' | 'godspeed' | 'shocker' | 'trickery' | 'perdition' | 'purgatory' | 'infernal-demise' | 'missiletoe' | 'her-world' | 'her-world-old' | 'relax' | 'forestall-desire' | 'broken-heart' | 'tribal' | 'endeavors' | 'slaybells' | 'my-horizon' | 'too-fest-old' | 'tetrabrachial' | 'my-horizon-wechidna' | 'our-horizon' | 'hedge' | 'manual-blast' | 'hollow' | 'hollow-old' |  'empty' | 'substantial' | 'digitalized' | 'gotta-go' | 'mania' | 'universal-collapse' | 'goddess' | 'color-blind' | 'corinthians' | 'genesis' | 'coulrophobia' | 'extreme-zap' | 'fake-baby' | 'extricate-hex' | 'gods-will' | 'gotta-go-red-glove' | 'heroes-and-villains' | 'melting' | 'too-far' | 'confronting' | 'too-far-alt' | 'animosity' | 'call-of-justice' | 'spike-trap' | 'jingle-hells' | 'hill-of-the-void' | 'endeavors-og' | 'thunderbolt' | 'color-blind-encore' | 'life-and-death' | 'bloodstorm' | 'test':
					add(blackFuck);
				...
				case 'fatality' | 'fatality-encore' | 'critical-error' | 'unresponsive':
					...
					fatal3 = new FlxSprite().loadGraphic(Paths.image("StartScreens/fatal_3", "exe"));
					fatal3.setGraphicSize(Std.int(fatal3.width * daPixelZoom));
					fatal3.scrollFactor.set();
					fatal3.updateHitbox();
					fatal3.screenCenter();
					fatal3.alpha = 0;
					fatal3.cameras = [camOther];
					fatal3.antialiasing = false;
					
					fatal2 = new FlxSprite().loadGraphic(Paths.image("StartScreens/fatal_2", "exe"));
					fatal2.setGraphicSize(Std.int(fatal2.width * daPixelZoom));
					fatal2.scrollFactor.set();
					fatal2.updateHitbox();
					fatal2.screenCenter();
					fatal2.alpha = 0;
					fatal2.cameras = [camOther];
					fatal2.antialiasing = false;
					
					fatal1 = new FlxSprite().loadGraphic(Paths.image("StartScreens/fatal_1", "exe"));
					fatal1.setGraphicSize(Std.int(fatal1.width * daPixelZoom));
					fatal1.scrollFactor.set();
					fatal1.updateHitbox();
					fatal1.screenCenter();
					fatal1.alpha = 0;
					fatal1.cameras = [camOther];
					fatal1.antialiasing = false;
		...
		MajinCountKick = new FlxSprite().loadGraphic(Paths.image('kickit', 'exe'));
		MajinCountKick.scrollFactor.set();
		MajinCountKick.updateHitbox();
		MajinCountKick.screenCenter();
		MajinCountKick.y -= 100;
		MajinCountKick.alpha = 0;
		MajinCountKick.cameras = [camOther];
		MajinCountKick.antialiasing = ClientPrefs.globalAntialiasing;
		...
		if (SONG.song.toLowerCase() == 'digitalized') {		
			preload('characters/XTerion/3D/xterion-first-gh');
		}
		
		if (SONG.song.toLowerCase() == 'fake-baby') {
			preload('JUMPSCARES/NMI', 'exe');
		}		
		
		if (SONG.song.toLowerCase() == 'sharpy-showdown') {
			preload('characters/BF/HD/bf-scared');
		}
		
		if (SONG.song.toLowerCase() == 'our-horizon') {
			preload('slash-splash');
			preload('characters/BF/HD/bf-scared');
			preload('JUMPSCARES/wechidna', 'exe');
		}
		
		if (SONG.song.toLowerCase() == 'forestall-desire') {
			preload('bgs/fd-bg/requital_slash');
			requitalSlashThing = new FlxSprite(-400, 300);
			requitalSlashThing.frames = Paths.getSparrowAtlas('bgs/fd-bg/requital_slash');
			requitalSlashThing.animation.addByPrefix('idle', 'requital_slash idle', 24, false);
			requitalSlashThing.scale.x = 0.3;
			requitalSlashThing.scale.y = 0.4;
			requitalSlashThing.visible = false;
			add(requitalSlashThing);
		}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function startVideo(name:String):Void {
		...
		if(foundFile) {
			...
			video.play(fileName + '/' + curSong.toLowerCase());
			video.volume = 300;
	...
	public var psychDialogue:objects.DialogueBoxPsych;
	...
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if(dialogueFile.dialogue.length > 0) {
			...
			psychDialogue = new objects.DialogueBoxPsych(dialogueFile, song);
	...
	var fatal3:FlxSprite;
	var fatal2:FlxSprite;
	var fatal1:FlxSprite;
				...
				startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer) {
					switch (swagCounter) {
						case 0:
							...
							fatal3.alpha = 1;
							FlxTween.tween(fatal3, {alpha: 0}, Conductor.crochet / 1000, {
						...
						case 1:
							...
							fatal2.alpha = 1;
							FlxTween.tween(fatal2, {alpha: 0}, Conductor.crochet / 1000, {
								ease: FlxEase.cubeInOut,
								onComplete: function(twn:FlxTween) {
									remove(fatal2);
									fatal2.destroy();
						...
						case 2:
							...
							fatal1.alpha = 1;
							FlxTween.tween(fatal1, {alpha: 0}, Conductor.crochet / 1000, {
								ease: FlxEase.cubeInOut,
								onComplete: function(twn:FlxTween) {
									remove(fatal1);
									fatal1.destroy();
	...
	public function startCountdown():Void
	{
		...
		if (startCount) {
			...
			if (SONG.isRing && ClientPrefs.isvpad || ClientPrefs.space == 'button' && MobileControls.mode != 'Keyboard')
			{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if(curStage == 'tetrabrachial-sl4sh')
				{
					if (!ClientPrefs.middleScroll)
						{
							playerStrums.forEach(function(spr:FlxSprite)
				...
				startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
				{
					...
					{
						if(combo >= 50 && boyfriend.animOffsets.exists('idle-alt'))
	...
	function startSong():Void {
		if (showCredits) {
			...
			if (FileSystem.exists(Paths.json(curSong.toLowerCase() + "/credits")))
				CreditsAuthors = File.getContent((Paths.json(curSong.toLowerCase() + "/credits"))).split("TIME")[0];
			else
				CreditsAuthors = File.getContent((Paths.json("unfinished-credits"))).split("TIME")[0];
			#else
			if (OpenFlAssets.exists(Paths.json(curSong.toLowerCase() + "/credits")))
				CreditsAuthors = OpenFlAssets.getText((Paths.json(curSong.toLowerCase() + "/credits"))).split("TIME")[0];
			else
				CreditsAuthors = OpenFlAssets.getText((Paths.json("unfinished-credits"))).split("TIME")[0];
			#end
			var CodeShit:Array<String> = CreditsAuthors.split("CODE")[1].split("ARTWORK")[0].split("
");
			var ArtShit:Array<String> = CreditsAuthors.split("ARTWORK")[1].split("MUSIC")[0].split("
");
			var MusicShit:Array<String> = CreditsAuthors.split("MUSIC")[1].split("CHARTING")[0].split("
");
			var ChartShit:Array<String> = CreditsAuthors.split("CHARTING")[1].split("TIME")[0].split("
");
			for (i in 0...CodeShit.length) {
				if (CodeShit[i].length > 2) {
					CreditsAuthorsCode = CreditsAuthorsCode + CodeShit[i];
					if (CodeShit.length > 2)
						CreditsAuthorsCode = CreditsAuthorsCode + ', ';
				}
			}
			for (i in 0...ArtShit.length) {
				if (ArtShit[i].length > 2) {
					CreditsAuthorsArt = CreditsAuthorsArt + ArtShit[i];
					if (ArtShit.length > 2)
						CreditsAuthorsArt = CreditsAuthorsArt + ', ';
				}
			}
			for (i in 0...MusicShit.length) {
				if (MusicShit[i].length > 2) {
					CreditsAuthorsMusic = CreditsAuthorsMusic + MusicShit[i];
					if (MusicShit.length > 2)
						CreditsAuthorsMusic = CreditsAuthorsMusic + ', ';
				}
			}
			for (i in 0...ChartShit.length) {
				if (ChartShit[i].length > 2) {
					CreditsAuthorsChart = CreditsAuthorsChart + ChartShit[i];
					if (ChartShit.length > 2)
						CreditsAuthorsChart = CreditsAuthorsChart + ', ';
				}
			}
			
			CreditsSplash = new FlxSprite(-760, 0).loadGraphic(Paths.image('credits/credits', 'shared'));
			CreditsSplash.scrollFactor.set(0, 0);
			CreditsSplash.antialiasing = true;
			CreditsSplash.cameras = [camOther];
			add(CreditsSplash);
			
			CreditsCodeSplash = new FlxSprite(-760, 0).loadGraphic(Paths.image('credits/code', 'shared'));
			CreditsCodeSplash.scrollFactor.set(0, 0);
			CreditsCodeSplash.antialiasing = true;
			CreditsCodeSplash.cameras = [camOther];
			add(CreditsCodeSplash);
		
			CreditsCodeSplashNicknames = new FlxText(-620, 110, 220, CreditsAuthorsCode);
			CreditsCodeSplashNicknames.setFormat(Paths.font('advanced.ttf'), 19, 0xFFFFFFFF);
			CreditsCodeSplashNicknames.antialiasing = true;
			CreditsCodeSplashNicknames.cameras = [camOther];
			add(CreditsCodeSplashNicknames);
			
			CreditsArtSplash = new FlxSprite(-760, 0).loadGraphic(Paths.image('credits/art', 'shared'));
			CreditsArtSplash.scrollFactor.set(0, 0);
			CreditsArtSplash.antialiasing = true;
			CreditsArtSplash.cameras = [camOther];
			add(CreditsArtSplash);
		
			CreditsArtSplashNicknames = new FlxText(-620, 246, 220, CreditsAuthorsArt);
			CreditsArtSplashNicknames.setFormat(Paths.font('advanced.ttf'), 19, 0xFFFFFFFF);
			CreditsArtSplashNicknames.antialiasing = true;
			CreditsArtSplashNicknames.cameras = [camOther];
			add(CreditsArtSplashNicknames);
			
			CreditsMusicSplash = new FlxSprite(-760, 0).loadGraphic(Paths.image('credits/music', 'shared'));
			CreditsMusicSplash.scrollFactor.set(0, 0);
			CreditsMusicSplash.antialiasing = true;
			CreditsMusicSplash.cameras = [camOther];
			add(CreditsMusicSplash);
		
			CreditsMusicSplashNicknames = new FlxText(-620, 382, 220, CreditsAuthorsMusic);
			CreditsMusicSplashNicknames.setFormat(Paths.font('advanced.ttf'), 19, 0xFFFFFFFF);
			CreditsMusicSplashNicknames.antialiasing = true;
			CreditsMusicSplashNicknames.cameras = [camOther];
			add(CreditsMusicSplashNicknames);
			
			CreditsChartSplash = new FlxSprite(-760, 0).loadGraphic(Paths.image('credits/chart', 'shared'));
			CreditsChartSplash.scrollFactor.set(0, 0);
			CreditsChartSplash.antialiasing = true;
			CreditsChartSplash.cameras = [camOther];
			add(CreditsChartSplash);
		
			CreditsChartSplashNicknames = new FlxText(-620, 518, 220, CreditsAuthorsChart);
			CreditsChartSplashNicknames.setFormat(Paths.font('advanced.ttf'), 19, 0xFFFFFFFF);
			CreditsChartSplashNicknames.antialiasing = true;
			CreditsChartSplashNicknames.cameras = [camOther];
			add(CreditsChartSplashNicknames);
			
			CreditsEtcSplash = new FlxSprite(-760, 0).loadGraphic(Paths.image('credits/etc', 'shared'));
			CreditsEtcSplash.scrollFactor.set(0, 0);
			CreditsEtcSplash.antialiasing = true;
			CreditsEtcSplash.cameras = [camOther];
			add(CreditsEtcSplash);
			
			CreditsEtcSplashMode = new FlxText(-620, 650, 275, '');
			CreditsEtcSplashMode.setFormat(Paths.font('advanced.ttf'), 23, 0xFFFFFFFF);
			CreditsEtcSplashMode.antialiasing = true;
			CreditsEtcSplashMode.cameras = [camOther];
			add(CreditsEtcSplashMode);
		
			var startDelay:Float = 2;
			var endDelay:Float = 2.4;
			#if desktop
			if (FileSystem.exists(Paths.json(curSong.toLowerCase() + "/credits")))
				startDelay = Std.parseFloat(File.getContent((Paths.json(curSong.toLowerCase() + "/credits"))).split("TIME")[1]);
			else
				startDelay = Std.parseFloat(File.getContent((Paths.json("unfinished-credits"))).split("TIME")[1]);
			#else
			if (OpenFlAssets.exists(Paths.json(curSong.toLowerCase() + "/credits")))
				startDelay = Std.parseFloat(OpenFlAssets.getText((Paths.json(curSong.toLowerCase() + "/credits"))).split("TIME")[1]);
			else
				startDelay = Std.parseFloat(OpenFlAssets.getText((Paths.json("unfinished-credits"))).split("TIME")[1]);
			#end
			creditsMove(startDelay, endDelay);
		}
		startingSong = false;
		...
		//popupWindow(520, 520, 500, 'Majin');
	}

	function creditsMove(inDelay:Float = 0, outDelay:Float = 0) {
		new FlxTimer().start(inDelay, function(In:FlxTimer) {
			FlxTween.tween(CreditsSplash, {x: 0}, 1.0, {ease: FlxEase.quintInOut});
			FlxTween.tween(CreditsCodeSplash, {x: 0}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.07});
			FlxTween.tween(CreditsCodeSplashNicknames, {x: 140}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.07});
			FlxTween.tween(CreditsArtSplash, {x: 0}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.14});
			FlxTween.tween(CreditsArtSplashNicknames, {x: 140}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.14});
			FlxTween.tween(CreditsMusicSplash, {x: 0}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.21});
			FlxTween.tween(CreditsMusicSplashNicknames, {x: 140}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.21});
			FlxTween.tween(CreditsChartSplash, {x: 0}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.28});
			FlxTween.tween(CreditsChartSplashNicknames, {x: 140}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.28});
			FlxTween.tween(CreditsEtcSplash, {x: 0}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.35});
			FlxTween.tween(CreditsEtcSplashMode, {x: 10}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.35});
			
			new FlxTimer().start(outDelay, function(Out:FlxTimer) {
				FlxTween.tween(CreditsSplash, {x: -760}, 1.0, {ease: FlxEase.quintInOut});
				FlxTween.tween(CreditsCodeSplash, {x: -760}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.07});
				FlxTween.tween(CreditsCodeSplashNicknames, {x: -620}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.07});
				FlxTween.tween(CreditsArtSplash, {x: -760}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.14});
				FlxTween.tween(CreditsArtSplashNicknames, {x: -620}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.14});
				FlxTween.tween(CreditsMusicSplash, {x: -760}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.21});
				FlxTween.tween(CreditsMusicSplashNicknames, {x: -620}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.21});
				FlxTween.tween(CreditsChartSplash, {x: -760}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.28});
				FlxTween.tween(CreditsChartSplashNicknames, {x: -620}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.28});
				FlxTween.tween(CreditsEtcSplash, {x: -710}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.35});
				FlxTween.tween(CreditsEtcSplashMode, {x: -1410}, 1.0, {ease: FlxEase.quintInOut, startDelay: 0.35});
			});
		});
	}
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...keyCount)
		{
			...
			if (fatalUI && player == 1) {
				...
				if ((SONG.player2 == "sunky" && player == 0) || (SONG.player2 == "sunky" && (player == 1 && SONG.player1 != 'sanic'))) { 
					...
					if (fatalUI && player == 0) {
						...
						if (SONG.player2 == "Furnace" && player == 0) {
							babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player,'prey_FURNACE');
						} else {	
							if (SONG.player1 == "prey-intro-idle" && player == 1) {
								babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player,'prey_SONIC');
							} else {	
								if (SONG.player1 == "fatal-error-playable" && player == 1 || SONG.player2 == "fatal-tetra" && player == 0) {
									babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player,'fatal_new');
									} else {
			...
			if (!isStoryMode && !skipArrowStartTween) {
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {alpha: targetAlpha}, 1, {ease: FlxEase.circOut, startDelay: 0.1 + (0.1 * i)});
			} else {
	...
	function colorsHandlerChange(color:FlxColor, ?lightness:Float = 0.6) {
		if (timeTxt != null && timeTxt.color != TimeBarColorTwo)
			timeTxt.color = color.getLightened(lightness);
		if (scoreTxt != null && scoreTxt.color != TimeBarColorTwo)
			scoreTxt.color = color.getLightened(lightness);
		if (botplayTxt != null && botplayTxt.color != TimeBarColorTwo)
			botplayTxt.color = color.getLightened(lightness);
		if (xenoEncMissesTxt != null && xenoEncUI && xenoEncMissesTxt.color != TimeBarColorTwo)
			xenoEncMissesTxt.color = color.getLightened(lightness);
		if (devoidUIhpTxt != null && devoidUI && devoidUIhpTxt.color != TimeBarColorTwo)
			devoidUIhpTxt.color = color.getLightened(lightness);
		if (devoidUImissesTxt != null && devoidUI && devoidUImissesTxt.color != TimeBarColorTwo)
			devoidUImissesTxt.color = color.getLightened(lightness);
	}
	...
	{
		#if !mobile
		if(SONG.song.toLowerCase() == 'universal-collapse' && curStep < 112) {
			if(FlxG.keys.justPressed.SPACE)
			{
				skipTxt.alpha = 0;
				PlayState.instance.clearNotesBefore(12000);
				PlayState.instance.setSongTime(12000);
			}
		}
		if(SONG.song.toLowerCase() == 'universal-collapse' && curStep >= 112)
			skipTxt.alpha = 0;
		...
		if (SONG.song.toLowerCase()=='prey' || SONG.song.toLowerCase()=='prey-good-future') {
			FlxG.camera.follow(camHit, LOCKON, 1);
			FlxG.collide(bfHitbox, hitboxesGrp, onPlayerStandingOnPlatform);
			FlxG.collide(dadHitbox, hitboxesGrp, onOppStandingOnPlatform);
			if(curStep <= 131) {
				boyfriendGroup.y = bfHitbox.y - 523;
			} else {
				boyfriendGroup.y = bfHitbox.y - 522;
			}
			boyfriendGroup.x = bfHitbox.x - 497;

			boyfriendGroup.maxVelocity.set(bfHitbox.speed, bfHitbox.maxFallSpeed);
			// trace("Boyfriend - drag: " + boyfriendGroup.drag.x + ", velocity: " + boyfriendGroup.velocity.x + ", maxVelocity: " + boyfriendGroup.maxVelocity);
			// trace("Hitbox - drag: " + bfHitbox.drag.x + ", velocity: " + bfHitbox.velocity.x + ", maxVelocity: " + bfHitbox.maxVelocity);

			if(curStep<256) {
				boyfriendGroup.velocity.x = 400;
				boyfriendGroup.drag.x = 400 * 4;
				dadGroup.velocity.x = 400;
				dadGroup.drag.x = 400 * 4;
				camHit.y = bfHitbox.y;
				camHit.speed = bfHitbox.speed;
			} else if(curStep >= 256 && curStep < 1787) {
				boyfriendGroup.velocity.x = 462;
				boyfriendGroup.drag.x = 462 * 4;
				dadGroup.velocity.x = 462;
				dadGroup.drag.x = 462 * 4;
			}

			if(curStep <= 256) { //begin
			} else if(curStep > 281 && curStep < 1536) { //singing p1
				camHit.y = bfHitbox.y-20;
			// } else if(curStep >= 1545 && curStep < 1620) {//cutscene 2 p1
			// } else if(curStep >= 1620 && curStep < 1788) { //cutscene 2 p2
			// } else if(curStep >= 1788 && curStep < 2448) { //singing p2
			// } else if(curStep >= 2448 && curStep < 2496) { //furnace part
			// } else if(curStep >= 2496 && curStep < 3335) { //singing again
			// } else if(curStep>=3335) { //last cutscene
			}

			if(curStep>=1545) {
				dadGroup.x = eggHitbox.x + 200;
				dadGroup.y = eggHitbox.y;
			} else {
				dadGroup.x = dadHitbox.x - 297;
				dadGroup.y = dadHitbox.y - 322;
			}

			if (curStep >= 1788) {
				dadGroup.velocity.x = dadHitbox.velocity.x + 200;
				dadGroup.drag.x = (dadHitbox.velocity.x + 200) * 4;
				boyfriendGroup.velocity.x = bfHitbox.velocity.x + 200;
				boyfriendGroup.drag.x = (bfHitbox.velocity.x + 200) * 4;
			}

			// trace("BFX: " + bfHitbox.x + ", BFY: " + bfHitbox.y);
			// trace("DADX: " + dadHitbox.x + ", DADY: " + dadHitbox.y);
		}
		callOnLuas('onUpdate', [elapsed]);

		if (autoChangeAllTextColor && !VHSui) {
			if (xenoEncUI) {
				colorsHandlerChange(TimeBarColorTwo, 0.7);
			} else { 
				colorsHandlerChange(TimeBarColorTwo, 0.5);
			}
		}
		...
		if (controls.EMOTE && boyfriend.animOffsets.exists('hey')) {
			triggerEventNote('Play Animation', 'hey', 'bf');
		}
		...
		if (SONG.song.toLowerCase()=='face-off') {
			switch(curStep)
			{
			case 421:
				videoPlaying = true;
		...
		if (controls.PAUSE && canPause == false) {
			trace('nuh uh');
		}
		if (controls.PAUSE && canPause == true) {
			trace('paused yeeee');
		}
		if (curStage == 'fatal-encore' || curStage == 'soink') {
			...
			if (dad.curCharacter == 'wechBeast') {
				...
				if (caShader.amount < -0.100) { 
					switch(boyfriend.animation.curAnim.name){
						case 'singUP', 'singDOWN', 'singLEFT', 'singRIGHT', 'singUP-alt', 'singDOWN-alt', 'singLEFT-alt', 'singRIGHT-alt': caShader.amount += 0.0150;
					}
				}
			}
			...
			switch(MarPressed) {
				...
				case 2:
					if (FlxG.keys.justPressed.B) {
						...
						gameovers.GameOverSubstate.loopSoundName = 'slash-gameover';
						gameovers.GameOverSubstate.endSoundName = 'slash-retry';
						gameovers.GameOverSubstate.characterName = 'bf-slash-mar-death';
						boyfriend.y -= 35;
						boyfriend.x += 175;
						boyfriend.y += 220;
						MarStarBroBF = true;
		...
		if (SONG.song.toLowerCase() == 'godspdee') {
		...
		if (curSong.toLowerCase() == 'godspeed')
		{
			...
			health -= healthDrop * (400 / ((ccap + 1) / 8) * ((songMisses + 1) / 1.9));
		...
		{	
			...
			if (targetSpeed - starvedSpeed < 0.2) starvedSpeed = targetSpeed;
			starvedSpeed = 75;
			GSsky.scrollX -= (starvedSpeed * GSsky.scrollFactor.x) * (elapsed/(1/20));
			if (!ClientPrefs.Optimization) GSbbbg.scrollX -= (starvedSpeed * GSbbbg.scrollFactor.x) * (elapsed/(1/60));
			GSbbg.scrollX -= (starvedSpeed * GSbbg.scrollFactor.x) * (elapsed/(1/60));
			GSbg.scrollX -= (starvedSpeed * GSbg.scrollFactor.x) * (elapsed/(1/120));
			if (!ClientPrefs.Optimization) GSplants.scrollX -= (starvedSpeed * GSplants.scrollFactor.x) * (elapsed/(1/200));
		}
		...
		if (FlxG.keys.pressed.SHIFT && startedCountdown && canPause) {
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				openSubState(new substates.SongDebugSubstate());
		...
		if (showCredits && !startingSong && !paused && startedCountdown) {
			CreditsEtcSplashMode.text = StringTools.replace(curSong.toUpperCase(), '-', ' ') + ' [' + (Math.floor(Math.floor((songLength - FlxG.sound.music.time - ClientPrefs.noteOffset) / 1000) / 60) + ':' + Math.floor((songLength - FlxG.sound.music.time - ClientPrefs.noteOffset) / 1000) % 60) +']\n'#if linux + detailsText #end;
			showCredits = false;
		}
		if (camZooming)
		{
		...
		if (charWindow) {
			@:privateAccess
			if (dad == null || dad._frame == null || dad._frame.frame == null) {
				trace("Frame or dad not initialized yet");
				return;
			}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				switch (SONG.song.toLowerCase()) {
					case 'prey' | 'prey-good-future': FlxG.camera.zoom = 1.2;
				...
				switch (SONG.song.toLowerCase()) {
					case 'too-fest', 'too-fest-old':
						openSubState(new gameovers.GameOverSanicSubstate());
					case 'forestall-desire':
						openSubState(new gameovers.GameOverRequitalSubstate());
					case 'melting', 'melting-old', 'confronting':
						openSubState(new gameovers.GameOverMelthogSubstate());
					case 'personel':
						openSubState(new gameovers.GameOverColdsteelSubstate());
					case 'personel-serious':
						openSubState(new gameovers.GameOverSeriousColdsteelSubstate());
					case 'my-horizon-wechidna':
						openSubState(new gameovers.GameOverWechidnaSubstate());
					case 'haze', 'haze-og', 'marauder':
						openSubState(new gameovers.GameOverGriatosSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0], boyfriend.getScreenPosition().y - boyfriend.positionArray[1], camFollowPos.x, camFollowPos.y));
					default:
						openSubState(new gameovers.GameOverSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0], boyfriend.getScreenPosition().y - boyfriend.positionArray[1], camFollowPos.x, camFollowPos.y));
				}
				#if linux
				var rpcF:String = ' (Charting Mode)';
				if (TranslationLanguages.Text.exists(rpcF) && ClientPrefs.Language != 'English')
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Camera_Tween_Zoom':
				camGameZooms = false;
				var length:Array<String> = value1.split(", ");
				new FlxTimer().start(Std.parseFloat(length[1]), function(lol:FlxTimer){
					camGameZooms = true;
					// trace(camGameZooms);
				});
				// trace("length array: " + length + " value1: " + value1);
				// trace("val 1 " + length[0]);
				// trace("val 2 " + length[1]);
			...
			case 'Play Animation':
				...
				switch (value1)
				{
					case 'dodge':
						if (char == boyfriend)
							requitalSlashThing.x = -500;
							requitalSlashThing.y = 150;
						if (char == gf)
							requitalSlashThing.x = -600;
							requitalSlashThing.y = 200;
			...
			case 'Play Video':
				playVideo(value1);
				videoTime = Std.parseInt(value2 + 0.5);
				canPause = false;
				videoPlaying = true;
			...
			case 'Clear Popups':
				while(objects.FatalPopup.popups.length>0)
					objects.FatalPopup.popups[0].close();
			case 'Fatality Popup':
				var value:Int = Std.parseInt(value1);
			...
			case 'Majin count':
				switch (Std.parseFloat(value1))
				{
					case 1:
						...
						FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + 0.2}, 0.7, {ease: FlxEase.cubeInOut});
						majinSaysFuck(4);
					case 2:
						FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + 0.2}, 0.7, {ease: FlxEase.cubeInOut});
						majinSaysFuck(3);
					...
					case 4:
						inCutscene = false;
						FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 0.5, {ease: FlxEase.cubeInOut});
						majinSaysFuck(1);
					case 5:
						inCutscene = false;
						FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 0., {ease: FlxEase.cubeInOut});
						majinSaysFuck(0);
					}
			case 'Majin spin':
	...
	public function moveCameraOnNote(isDad:Bool, ?direction:String = "")
	{
		var addX:Float = 0;
		var addY:Float = 0;
		if (direction == 'singUP' || direction == 'singUP-alt') 
			addY -= 20;
		if (direction == 'singDOWN' || direction == 'singDOWN-alt') 
			addY += 20;
		if (direction == 'singLEFT' || direction == 'singLEFT-alt') 
			addX -= 20;
		if (direction == 'singRIGHT' || direction == 'singRIGHT-alt') 
			addX += 20;
	...
	{	
		...
		gameovers.GameOverSubstate.characterName = 'bf-fleetway-die';
		gameovers.GameOverSubstate.deathSoundName = 'fleetway-laser';
		gameovers.GameOverSubstate.loopSoundName = 'fleetway-gameover';
		...
		new FlxTimer().start(0.90, function(a:FlxTimer)
		{
			new FlxTimer().start(0.2, function(a:FlxTimer) {
				...
				new FlxTimer().start(0.1, function(a:FlxTimer) { canDodge = false; });
			});
			new FlxTimer().start(0.3, function(a:FlxTimer)
			{
				dodgething.visible = false;
				triggerEventNote('Change Character', '1', 'fleetway');
				gameovers.GameOverSubstate.characterName = 'bf';
				gameovers.GameOverSubstate.deathSoundName = 'fleetway-death';
				gameovers.GameOverSubstate.loopSoundName = 'fleetway-gameover';
				gameovers.GameOverSubstate.endSoundName = 'fleetway-retry';
				GameOverCameraMove = true;
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if ((SONG.song.toLowerCase() == 'triple-trouble' && ClientPrefs.storyProgressExe <= 3 && isStory && !practiceMode && !cpuControlled) || (SONG.song.toLowerCase() == 'goddess' && ClientPrefs.storyProgressAlt <= 4 && !ClientPrefs.youSawSoundTestCutscene && isStory && !practiceMode && !cpuControlled) || (SONG.song.toLowerCase() == 'malediction' && isSound)) {
				if (SONG.song.toLowerCase() == 'triple-trouble') {
					FlxG.mouse.visible = false;
					Main.InPlaystate = false;
					MusicBeatState.switchState(new vlc.Cutscenes('tt-final'));
				}
				if (SONG.song.toLowerCase() == 'goddess') {
					FlxG.mouse.visible = false;
					Main.InPlaystate = false;
					ClientPrefs.youSawSoundTestCutscene = true;
					MusicBeatState.switchState(new vlc.Cutscenes('STCodes'));
				}
				if (SONG.song.toLowerCase() == 'malediction') {
					FlxG.sound.music.stop();	
					cancelMusicFadeTween();
	...
	function updateSonicScore(){
		...
		if(seperatedScore.length<scoreNumbers.length){
			for(idx in seperatedScore.length...scoreNumbers.length){
				if(hudStyle == 'chaotix' || hudStyle == 'chotix' || hudStyle == 'sonic1' || hudStyle == 'sonic2' || hudStyle == 'sonic3' || hudStyle == 'soniccd'){
	...
	function doGhostAnim(char:String, animToPlay:String, ?minusAlpha:Float = 0.35)
	{
		...
		if (player.visible && player.alpha > 0.4) {
			...
			var json:objects.Character.CharacterFile = cast Json.parse(File.getContent(path));
			var animationsArray:Array<objects.Character.AnimArray> = json.animations;
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		} else if(daNote.noteType == 'Bullet Note'){
			health = health - 0.5;
			songMisses += 1;
			healthDrop += 0.00025;
			dropTime = 3;
		...
		if(char != null && char.hasMissAnimations)
		{
			...
			if (SONG.isRing)
			{
				...
				{
					case 0:
						if (missStack <= 1 && prevCombo >= 50 && boyfriend.animOffsets.exists('singLEFTmiss-alt'))
							daAlt = '-alt';
						animToPlay = 'singLEFTmiss' + daAlt;
					case 1:
						if (missStack <= 1 && prevCombo >= 50 && boyfriend.animOffsets.exists('singDOWNmiss-alt'))
							daAlt = '-alt';
						animToPlay = 'singDOWNmiss' + daAlt;
					case 2:
						animToPlay = '';
					case 3:
						if (missStack <= 1 && prevCombo >= 50 && boyfriend.animOffsets.exists('singUPmiss-alt'))
							daAlt = '-alt';
						animToPlay = 'singUPmiss' + daAlt;
					case 4:
						if (missStack <= 1 && prevCombo >= 50 && boyfriend.animOffsets.exists('singRIGHTmiss-alt'))
							daAlt = '-alt';
			...
			trace("AnimToPlay: " + animToPlay + " Miss Stack: " + missStack + " Previous Combo: " + prevCombo + " Alt thing: " + daAlt);
		...
		switch (SONG.song.toLowerCase()) {
			...
			default:
				if (!customIconAndName) {
					if(!scoreRandom){
						if(curStep>=1148 && curStep<=1232 && SONG.song.toLowerCase() == 'cycles')
							lime.app.Application.current.window.title = "FNF': Restoration Of Sonic.exe | Song: " + SONG.song.toLowerCase() + " | DIGA BYE BYE";
						else	
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Knife Note':
						health -= 0.3;
						maxHealth -= 0.3;
						requitalSlashThing.animation.play('idle');
						requitalSlashThing.visible = true;
						if (requitalSlash)
							requitalSlashTimer.cancel();
						requitalSlash = true;
						requitalSlashTimer = new FlxTimer().start(0.2, function(tmr:FlxTimer) {
							requitalSlash = false;
							requitalSlashThing.visible = false;
						});
						
						trace('Max Health Allowed: ' + maxHealth);
						if (healthLock)
							healthLockTimer.cancel();
						healthLockTimer = new FlxTimer().start(15, function(tmr:FlxTimer) {
							healthLock = false;
						});
						healthLock = true;
					...
					case 'Fatal Note', 'Fatal Note Alt':
						songMisses += 1;
			...
			if(!note.noAnimation) {
				...
				if (SONG.isRing) {
					switch (Math.abs(note.noteData)) {
						case 0:
							if (combo >= 50 && boyfriend.animOffsets.exists('singLEFT-alt') && SONG.player1 != 'bf-sunkeh')
								animToPlay = 'singLEFT-alt';
							else
								animToPlay = 'singLEFT';
						case 1:
							if (combo >= 50 && boyfriend.animOffsets.exists('singDOWN-alt') && SONG.player1 != 'bf-sunkeh')
						...
						case 2:
							animToPlay = 'daRing';
						case 3:
							if (combo >= 50 && boyfriend.animOffsets.exists('singUP-alt') && SONG.player1 != 'bf-sunkeh')
								animToPlay = 'singUP-alt';
							else
								animToPlay = 'singUP';
						case 4:
							if (combo >= 50 && boyfriend.animOffsets.exists('singRIGHT-alt') && SONG.player1 != 'bf-sunkeh')
				...
				if(note.noteType == 'GF and BF')
					triggerEventNote('Play Animation', animToPlay, 'bf');
					//triggerEventNote('Play Animation', animToPlay, 'gf');
	...
	function playVideo(name:String){
		...
		canPause = false;
		videoPlaying = true;
		new FlxTimer().start(videoTime, function(tmr:FlxTimer)
		{
			video.alpha = 0;
			canPause = true;
			videoPlaying = false;
	...
	function majinSaysFuck(numb:Int):Void
		{
			...
			{
				case 4:
					MajinCountThree.alpha = 0.8;
					add(MajinCountThree);
					FlxTween.tween(MajinCountThree, {y: MajinCountThree.y + 100, alpha: 0, "scale.x": 1.05, "scale.y": 1.05}, Conductor.crochet / 450, {
				...
				case 3:
					MajinCountTwo.alpha = 0.8;
					add(MajinCountTwo);
					FlxTween.tween(MajinCountTwo, {y: MajinCountTwo.y + 100, alpha: 0, "scale.x": 1.05, "scale.y": 1.05}, Conductor.crochet / 450, {
						ease: FlxEase.cubeOut,
						onComplete: function(twn:FlxTween)
						{
				...
				case 2:
					MajinCountOne.alpha = 0.8;
					add(MajinCountOne);
					FlxTween.tween(MajinCountOne, {y: MajinCountOne.y + 100, alpha: 0, "scale.x": 1.05, "scale.y": 1.05}, Conductor.crochet / 450, {
						ease: FlxEase.cubeOut,
						onComplete: function(twn:FlxTween)
						{
				...
				case 1:
					MajinCountGo.alpha = 0.8;
					add(MajinCountGo);
					FlxTween.tween(MajinCountGo, {y: MajinCountGo.y + 100, alpha: 0, "scale.x": 1.05, "scale.y": 1.05}, Conductor.crochet / 450, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
				...
				case 0:
					MajinCountKick.alpha = 0.8;
					add(MajinCountKick);
					FlxTween.tween(MajinCountKick, {y: MajinCountKick.y + 100, alpha: 0, "scale.x": 1.05, "scale.y": 1.05}, Conductor.crochet / 450, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							MajinCountKick.destroy();
						}
					});
			}
		}
	...
	override function stepHit()
	{
		...
		if (curStep == 992 && SONG.song.toLowerCase() == 'malediction') {
			if (songMisses <= 10)
				curseEnding = 1;
			if (songMisses > 10)
				curseEnding = 2;
		...
		if (SONG.song.toLowerCase() == 'flashcard') {
			switch (curStep) {
				case 112:
					FlxTween.tween(blackFuck, {alpha: 1}, 0.05);
					FlxTween.tween(startCircle, {alpha: 1}, 0.05);
					FlxTween.tween(startText, {alpha: 1}, 0.05);
					new FlxTimer().start(0.4, function(tmr:FlxTimer)
					{
						FlxTween.tween(startCircle, {x: 0}, 0.3);
						FlxTween.tween(startText, {x: 0}, 0.3);
					});

					new FlxTimer().start(1.5, function(tmr:FlxTimer)
					{
						FlxTween.tween(blackFuck, {alpha: 0}, 0.5, {
							onComplete: function(twn:FlxTween)
							{
								remove(blackFuck);
								blackFuck.destroy();
						...
						FlxTween.tween(startCircle, {alpha: 0}, 0.5, {
							onComplete: function(twn:FlxTween)
							{
								remove(startCircle);
								startCircle.destroy();
						...
						FlxTween.tween(startText, {alpha: 0}, 0.5, {
							onComplete: function(twn:FlxTween)
							{
								remove(startText);
								startText.destroy();
		...
		if (SONG.song.toLowerCase() == 'our-horizon') {
			switch (curStep) {
				...
				case 2848:
					...
					gameovers.GameOverSubstate.loopSoundName = 'gameOver';
					gameovers.GameOverSubstate.endSoundName = 'gameOverEnd';
					gameovers.GameOverSubstate.characterName = 'bf-holding-gf-dead';
					gf.alpha = 0;
					snapCamFollowToPos(700, 900);
					defaultCamZoom = 0.7;
					dad.setPosition(200, 700);
		...
		if (curStage == 'sunkyandsanic' && SONG.song.toLowerCase()=='nostalgic-duo')
		{
			switch (curStep)
			{
				case 1:
				...
				case 144: //x
					if (!ClientPrefs.PotatoOptimization) {
						switch(FlxG.random.int(1, 3)){
							case 1:
								FlxTween.tween(cereal, {x: 2350}, 12.5);
							case 2:
								FlxTween.tween(munch, {x: 2350}, 12.5);
							case 3:
								FlxTween.tween(pose, {x: 2350}, 12.5);
						}
					}
				case 287:
					if (!ClientPrefs.PotatoOptimization) {
				...
				case 384:
					if (!ClientPrefs.PotatoOptimization) {
						FlxTween.tween(duo, {y: 1150}, 9.8);
					}
				}
			}	
		if (curStage == 'stage' && SONG.song.toLowerCase()=='test')
		{
			switch(curStep)
			{
				case 496:
					if (!ClientPrefs.PotatoOptimization) {
						FlxTween.tween(duo, {y: 1150}, 2);
					}
					duo.x = -400;
					duo.y = -1000;
					duo.scale.set(0.4, 0.4);
		...
		if (curStage == 'sunkStage' && SONG.song.toLowerCase()=='milk')
		{
				...
				switch (curStep)
					{
						case 288:
							...
							FlxTween.tween(lavasl4sh, {y: -40}, 1.5, {ease: FlxEase.cubeInOut, onComplete: function(tween){
								FlxTween.tween(lavasl4sh, {y: -25}, 0.85, {ease: FlxEase.backInOut, type: 4});
							}});

							for (sl4shPart in [eyesback, wallsl4sh, eyesfront, pillers, floorsl4sh])
								FlxTween.tween(sl4shPart, {alpha: 1}, 1.5);

							for (char in [dad, boyfriend])
								FlxTween.color(char, 1.5, char.color, FlxColor.WHITE);

						case 1439:
							defaultCamZoom = 0.75;

							FlxTween.cancelTweensOf(lavasl4sh);
							FlxTween.tween(lavasl4sh, {y: 500}, 1.5, {ease: FlxEase.cubeInOut});

							for (sl4shPart in [eyesback, wallsl4sh, eyesfront, pillers, floorsl4sh])
								FlxTween.tween(sl4shPart, {alpha: 0.15}, 1.5);

							for (char in [dad, boyfriend])
								FlxTween.color(char, 1.5, char.color, 0xFF414141);
			...
			if (SONG.song.toLowerCase() == 'cycles') {
				switch (curStep) {
					case 1148:
						lime.app.Application.current.window.title += " | DIGA BYE BYE";
				}
			}
			if (SONG.song.toLowerCase() == 'prey' || SONG.song.toLowerCase()=='prey-good-future')
			{
				switch (curStep)
					{
						...
						case 257:
							...
							dadHitbox.speed = 800;
							camHit.speed = 400;
							FlxTween.tween(dadHitbox, {speed: 512}, 1.2, {ease: FlxEase.cubeInOut});
							FlxTween.tween(camHit, {y: bfHitbox.y - 20}, 1.2, {ease: FlxEase.cubeInOut});
							FlxTween.tween(camHit, {speed: 512}, 1.2, {ease: FlxEase.cubeInOut});
						case 1530:
							FlxTween.tween(camHUD, {alpha: 0}, 0.75,{ease: FlxEase.cubeInOut});
						case 1536:
							camHit.speed = 682;
							FlxTween.tween(camHit, {y: bfHitbox.y}, 1, {ease: FlxEase.cubeInOut});
							FlxTween.tween(camHit, {speed: 512}, 1, {ease: FlxEase.cubeInOut});
						case 1505:
							FlxTween.tween(dadHitbox, {speed: 0}, 5, {ease: FlxEase.cubeInOut});
							FlxTween.angle(dad, 0, -180, 5, {ease: FlxEase.cubeInOut});
						case 1542:
							dadGroup.visible = false;
						case 1545:
							eggHitbox.y = eggHitbox.y + 210;
							cinematicBars(true);
							dad.x -= 500;
							dad.y += 100;
						case 1548:
						...
						case 1570:
							eggHitbox.speed = 2500;
							FlxTween.tween(eggHitbox, {speed: 512}, 1.2, {ease: FlxEase.cubeOut});
						case 1616:
							camHit.speed = 1102;
							FlxTween.tween(camHit, {y: eggHitbox.y}, 1.2, {ease: FlxEase.cubeOut});
							FlxTween.tween(camHit, {speed: 512}, 1.2, {ease: FlxEase.cubeOut});
						case 1780:
						...
						case 1792:
							camHit.speed -= 400;
							FlxTween.tween(camHit, {y: bfHitbox.y-30}, 0.6, {ease: FlxEase.cubeOut});
							FlxTween.tween(camHit, {speed: 800}, 0.6, {ease: FlxEase.cubeOut});
						case 2432:
							// FlxTween.tween(stardustFurnace, {x: 3000}, 9);
							camHit.speed -= 400;
							FlxTween.tween(camHit, {speed: 800}, 1.2, {ease: FlxEase.cubeOut});
						case 2440:
							FlxTween.tween(camHit, {y: bfHitbox.y + 100}, 3, {ease: FlxEase.cubeOut});
						case 2525:
							camHit.speed += 400;
							FlxTween.tween(camHit, {y: bfHitbox.y-30}, 1.2, {ease: FlxEase.cubeOut});
							FlxTween.tween(camHit, {speed: 800}, 1.2, {ease: FlxEase.cubeOut});
						case 3328:
							camHit.speed -= 400;	
							FlxTween.tween(camHUD, {alpha: 0}, 1,{ease: FlxEase.cubeInOut});
							// FlxTween.tween(dad, {x: -300}, 4,{ease: FlxEase.cubeInOut});
							FlxTween.tween(camHit, {y: bfHitbox.y}, 1.2, {ease: FlxEase.cubeOut});
							FlxTween.tween(camHit, {speed: 800}, 1.2, {ease: FlxEase.cubeOut});
						case 3335:
						...
						case 3364:
							cinematicBars(true);
							var gotcha:FlxSprite = new FlxSprite(boyfriend.x + 1500, boyfriend.y + 555).loadGraphic(Paths.image('furnace_gotcha', 'exe'));
							gotcha.setGraphicSize(Std.int(gotcha.width * 4));
	...
	//addHitboxOnColor(largeSprite, FlxColor.RED); // Replace RED with your target color
    function addHitboxOnColor(sprite:FlxSprite, targetColor:FlxColor):Void {
        var pixels = sprite.pixels; // Get the pixel data from the sprite

        for (y in 0...pixels.height) {
            for (x in 0...pixels.width) {
                // Get the color of the current pixel
                var pixelColor:FlxColor = pixels.getPixel(x, y);

                // If the pixel matches the target color, create a new sprite
                if (pixelColor == targetColor) {
                    var newSprite = new FlxSprite(sprite.x + x, sprite.y + y);
                    newSprite.makeGraphic(1, 1, targetColor); // Create a 1x1 sprite
                    add(newSprite);
                }
            }
        }
	...
	function generationAlgorithm(riggedGeneration:Bool, thingy:Int, autoGenerate:Bool, specifiedNum:Int) {
		var toRemove:Array<Int> = [];
		var hitbox:String;
		var hitboxNumber:Array<Int> = [];
		if (autoGenerate && !riggedGeneration) {
			switch(thingy) {
				case 0,1:
					Options = [1,1,1,1,1,12,13,15,18,19,24,24,24,24,24,33,33,33,33];
				case 2:
					Options = [1,12,13,13,13,13,13,15,18,19,20];
				case 4:
					Options = [6,6,6,6,6,14,14,30,30,30];
				case 6:
					Options = [6,6,6,6,4,4,14,31,31,31,30];
				case 11:
					Options = [11,11,13,13,13,16,16,21,21,21,26,26,26,26];
				case 12:
					Options = [1,1,1,1,15,19,20];
				case 13:
					Options = [1,1,13,11,16,16,16,16,16,13,13,26,24,28,33];
				case 14:
					Options = [6,6,6,6,6,14,14,27,30];
				case 15,19:
					Options = [1,1,1,1,1,20,20,24,24,24,33,33];
				case 16:
					Options = [11,11,11,11,11,26,26];
				case 18:
					Options = [1,1,1,13,13,13,13,13,13,24,24,24,33,33,33];
				case 20:
					Options = [13];
				case 21:
					Options = [11,11,11,11,26,26,26];
				case 24:
					Options = [4,4,4,6,6,6,6];
				case 26:
					Options = [1,13,15,15,18,18,18,19,19,19,19,19,20,20];
				case 27:
					Options = [4,4,4,6,6,6,6,14];
				case 28:
					Options = [13];
				// case 29:
				// 	Options = [4,11];
				case 30:
					Options = [14];
				case 31:
					Options = [1,1,1,1,12,12,13,15,18,19,20];
				case 33:
					Options = [11];
			}
					
			if (thingy == 13 && level == 4)
				toRemove = [1,1,7,24,33];
			else if (thingy == 13 && level == 2)
				toRemove = [11,16,16,16,16,16,26];
			else if (thingy == 14 && level == 3)
				toRemove = [6,6,6,6,6,30];
			else if (thingy == 14 && level == 1)
				toRemove = [27];
			else if (thingy == 29 && level == 4)
				toRemove = [4];
			else if (thingy == 29 && level == 2)
				toRemove = [11];

			for (i in 0...toRemove.length) {
				Options.remove(toRemove[i]);
			}
			...
			if (thingy == 28 && level == 2)
				level = 4;
			else if (thingy == 28 && level == 4)
				level = 2;
			else if (thingy == 29 && level == 2)
				level = 1;
			else if (thingy == 25 && level == 4)
				level = 2;
			else if (thingy == 25 && level == 3)
				level = 1;
			else if (thingy == 32 && level == 1)
				level == 3;
			else if (thingy == 32 && level == 2)
				level == 4;
			
			var path1:String;
			if (thingy <= 23)
				path1 = "flat";
			else if (thingy > 23 && thingy <= 27)
				path1 = "down";
			else if (thingy > 27 && thingy <= 29)
				path1 = "misc";
			else
				path1 = "up";
					
			switch(thingy) {
				case 2,4,8,9,10,13,14,17,23,24,26,27,30,31,33:
					hitboxNumber = [1,2];
				// case 7:
				// 	hitboxNumber = [1,2,3];
				case 25,28,32:
					hitboxNumber = [1,2,3,4];
				// case 29:
				// 	hitboxNumber = [1,2,3,4,5];
				default:
					hitboxNumber = [1];
			}
			trace("Auto Generation Init");
			shuffleArray(Options);
			final tile = new FlxSprite(300+dis, 300).loadGraphic(Paths.image("tiles/"+path1+"/tile"+thingy, "shared"), false, 256, 256);
			tile.immovable = true;
			tile.antialiasing = false;
			platformsGrp.add(tile);
					
			for (j in 0...hitboxNumber.length) {
				final hitboxes = new FlxSprite(300 + dis, 300).loadGraphic(Paths.image("hitboxes/" + path1 + "/tile" + thingy + "-" + hitboxNumber[j], "shared"), false, 256, 256);
				hitboxes.immovable = true;
				hitboxes.antialiasing = false;
				
				if (hitboxes.pixels != null) {
					var bounds = getAlphaBounds(hitboxes.pixels);
					hitboxes.setSize(bounds.width, bounds.height); // Adjust hitbox size
					hitboxes.offset.set(bounds.x, bounds.y); // Adjust hitbox offset
					hitboxes.x += bounds.x;
					hitboxes.y += bounds.y;
					hitboxes.visible = false;
				}
				hitboxes.ID = thingy;
				hitboxesGrp.add(hitboxes);
			}
			dis+=256;
			counter++;
		
			if (counter <= 170)
				generationAlgorithm(false, Options[0], true, 0);
		} else if(!autoGenerate && !riggedGeneration) {
			for (j in 0...specifiedNum) {
				trace("Flat Generation Init");
				thingy = FlxG.random.int(0,1);
				hitboxNumber = [1];
				var path1:String;
				if (thingy <= 23)
					path1 = "flat";
				...
				for(k in 0...hitboxNumber.length) {
					final hitboxes = new FlxSprite(300 + dis, 300).loadGraphic(Paths.image("hitboxes/" + path1 + "/tile" + thingy + "-" + hitboxNumber[k], "shared"), false, 256, 256);
					hitboxes.immovable = true;
					hitboxes.antialiasing = false;
			...
			generationAlgorithm(false, 1, true, 0);
		} else if (riggedGeneration && !autoGenerate) {
			for (f in 0...1) {
				trace("Rigged Generation Init");
				if (level == 1) {
					thingy = 31;
					hitboxNumber = [1,2];
					level = 2;
				} else if (level == 2) {
					generationAlgorithm(false, 1, false, 440);
					return;
				} else if (level == 3) {
					thingy = 27;
					hitboxNumber = [1,2];
				...
				for(g in 0...hitboxNumber.length) {
					final hitboxes = new FlxSprite(300 + dis, 300).loadGraphic(Paths.image("hitboxes/" + path1 + "/tile" + thingy + "-" + hitboxNumber[g], "shared"), false, 256, 256);
					hitboxes.immovable = true;
					hitboxes.antialiasing = false;
	...
	function createLevel() {
        var dis:Int = 0;
        platformsGrp = new FlxSpriteGroup();
        hitboxesGrp = new FlxSpriteGroup();
        var option:Int = FlxG.random.int(1,9);
        generationAlgorithm(false, 1, false, 40);
    }

	var doTween:Bool = true;
	var currentPlat:Int = 0;

	function onPlayerStandingOnPlatform(player:FlxObject, hitbox:FlxObject):Void {
		// `platform` is the specific hitbox/platform the player is overlapping
		if (currentPlat != hitbox.ID) {
			//trace("Player is on hitbox with ID: " + currentPlat);
			currentPlat = hitbox.ID;
		}
		if (currentPlat == 24 && doTween || currentPlat == 29 && doTween && level == 2) {
			bfHitbox.gravity = 0;
			doTween = false;
			new FlxTimer().start(0.1*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(bfHitbox, {y: 433}, 0.07*speedMultiplier, {ease: FlxEase.quadIn});//
				new FlxTimer().start(0.07*speedMultiplier, function(spr:FlxTimer) {
					FlxTween.tween(bfHitbox, {y: 487}, 0.28*speedMultiplier, {ease: FlxEase.linear});
					new FlxTimer().start(0.28*speedMultiplier, function(spr:FlxTimer) {
						FlxTween.tween(bfHitbox, {y: 492}, 0.05*speedMultiplier, {ease: FlxEase.quadOut});//
						new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
							doTween = true;
							bfHitbox.gravity = 600;
							level = 1;
						});
					});
				});
			});
		} else if (currentPlat == 33 && doTween) {
			bfHitbox.gravity = 0;
			doTween = false;
			FlxTween.tween(bfHitbox, {y: 300}, 0.5*speedMultiplier, {ease: FlxEase.quadInOut});
			new FlxTimer().start(0.7*speedMultiplier, function(spr:FlxTimer) {
				doTween = true;
				bfHitbox.gravity = 600;
				level = 4;
			});
		} else if (currentPlat == 26 && doTween) {
			bfHitbox.gravity = 0;
			doTween = false;
			FlxTween.tween(bfHitbox, {y: 428}, 0.5*speedMultiplier, {ease: FlxEase.quadInOut});
			new FlxTimer().start(0.7*speedMultiplier, function(spr:FlxTimer) {
				doTween = true;
				bfHitbox.gravity = 600;
				level = 2;
			});
		} else if (currentPlat == 30 && doTween) {
			doTween = false;
			bfHitbox.gravity = 0;
			FlxTween.tween(bfHitbox, {y: 404}, 0.25*speedMultiplier, {ease: FlxEase.quadIn});
			new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
				trace("Done");
				FlxTween.tween(bfHitbox, {y: 364}, 0.27*speedMultiplier, {ease: FlxEase.quadOut});
				new FlxTimer().start(0.47*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					doTween = true;
					bfHitbox.gravity = 600;
					level = 3;
		...
		} else if (currentPlat == 28 && doTween && level == 2) {
			doTween = false;
			bfHitbox.gravity = 0;
			FlxTween.tween(bfHitbox, {y: 340}, 0.25*speedMultiplier, {ease: FlxEase.quadIn});
			new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
				trace("Done");
				FlxTween.tween(bfHitbox, {y: 300}, 0.27*speedMultiplier, {ease: FlxEase.quadOut});
				new FlxTimer().start(1*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					doTween = true;
					bfHitbox.gravity = 600;
					level = 4;
		...
		} else if (currentPlat == 28 && doTween && level == 4) {
			doTween = false;
			bfHitbox.gravity = 0;
			new FlxTimer().start(0.05*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(bfHitbox, {y: 340}, 0.21*speedMultiplier, {ease: FlxEase.quadIn});
				new FlxTimer().start(0.21*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					FlxTween.tween(bfHitbox, {y: 428}, 0.25*speedMultiplier, {ease: FlxEase.quadOut});
					new FlxTimer().start(1*speedMultiplier, function(spr:FlxTimer) {
						trace("Done");
						doTween = true;
						bfHitbox.gravity = 600;
		...
		} else if (currentPlat == 27 && doTween) {
			doTween = false;
			bfHitbox.gravity = 0;
			new FlxTimer().start(0.05*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(bfHitbox, {y: 404}, 0.21*speedMultiplier, {ease: FlxEase.quadIn});
				new FlxTimer().start(0.21*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					FlxTween.tween(bfHitbox, {y: 492}, 0.25*speedMultiplier, {ease: FlxEase.quadOut});
					new FlxTimer().start(0.45*speedMultiplier, function(spr:FlxTimer) {
						trace("Done");
						doTween = true;
						bfHitbox.gravity = 600;
						level = 1;
		...
		} else if (currentPlat == 31 && doTween) {
			bfHitbox.gravity = 0;
			doTween = false;
			FlxTween.tween(bfHitbox, {y: 487}, 0.07*speedMultiplier, {ease: FlxEase.quadIn});
			new FlxTimer().start(0.07*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(bfHitbox, {y: 433}, 0.28*speedMultiplier, {ease: FlxEase.linear});
				new FlxTimer().start(0.28*speedMultiplier, function(spr:FlxTimer) {
					FlxTween.tween(bfHitbox, {y: 428}, 0.05*speedMultiplier, {ease: FlxEase.quadOut});
					new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
						doTween = true;
						bfHitbox.gravity = 600;
						level = 2;
	...
	var doTweenDad:Bool = true;
	var currentPlatDad:Int = 0;

	function onOppStandingOnPlatform(player2:FlxObject, hitbox2:FlxObject):Void {
		// `platform` is the specific hitbox/platform the player is overlapping
				
		if (currentPlatDad == 7) {
			dadHitbox.gravity = 0;
			trace(dadHitbox.gravity + " " + currentPlatDad);
			new FlxTimer().start(1.3, function(tmr:FlxTimer) {
				dadHitbox.gravity = 600;
				trace(dadHitbox.gravity);
			});
		}

		if (currentPlatDad != hitbox2.ID) {
			//trace("Opp is on hitbox with ID: " + currentPlatDad);
			currentPlatDad = hitbox2.ID;
		}
		if (currentPlatDad == 24 && doTweenDad || currentPlatDad == 29 && doTweenDad && levelDad == 2) {
			dadHitbox.gravity = 0;
			doTweenDad = false;
			new FlxTimer().start(0.1*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(dadHitbox, {y: 433}, 0.07*speedMultiplier, {ease: FlxEase.quadIn});
				new FlxTimer().start(0.07*speedMultiplier, function(spr:FlxTimer) {
					FlxTween.tween(dadHitbox, {y: 487}, 0.28*speedMultiplier, {ease: FlxEase.linear});
					new FlxTimer().start(0.28*speedMultiplier, function(spr:FlxTimer) {
						FlxTween.tween(dadHitbox, {y: 492}, 0.05*speedMultiplier, {ease: FlxEase.quadOut});
						new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
							doTweenDad = true;
							dadHitbox.gravity = 600;
							levelDad = 1;
		...
		} else if (currentPlatDad == 33 && doTweenDad) {
			dadHitbox.gravity = 0;
			doTweenDad = false;
			FlxTween.tween(dadHitbox, {y: 300}, 0.5*speedMultiplier, {ease: FlxEase.quadInOut});
			new FlxTimer().start(0.7*speedMultiplier, function(spr:FlxTimer) {
				doTweenDad = true;
				dadHitbox.gravity = 600;
				levelDad = 4;
			});
		} else if (currentPlatDad == 26 && doTweenDad) {
			dadHitbox.gravity = 0;
			doTweenDad = false;
			FlxTween.tween(dadHitbox, {y: 428}, 0.5*speedMultiplier, {ease: FlxEase.quadInOut});
			new FlxTimer().start(0.7*speedMultiplier, function(spr:FlxTimer) {
				doTweenDad = true;
				dadHitbox.gravity = 600;
				levelDad = 2;
			});
		} else if (currentPlatDad == 30 && doTweenDad) {
			doTweenDad = false;
			dadHitbox.gravity = 0;
			FlxTween.tween(dadHitbox, {y: 404}, 0.25*speedMultiplier, {ease: FlxEase.quadIn});
			new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
				trace("Done");
				FlxTween.tween(dadHitbox, {y: 364}, 0.27*speedMultiplier, {ease: FlxEase.quadOut});
				new FlxTimer().start(0.47*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					doTweenDad = true;
					dadHitbox.gravity = 600;
		...
		} else if (currentPlatDad == 28 && doTweenDad && levelDad == 2) {
			doTweenDad = false;
			dadHitbox.gravity = 0;
			FlxTween.tween(dadHitbox, {y: 340}, 0.25*speedMultiplier, {ease: FlxEase.quadIn});
			new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
				trace("Done");
				FlxTween.tween(dadHitbox, {y: 300}, 0.27*speedMultiplier, {ease: FlxEase.quadOut});
				new FlxTimer().start(1*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					doTweenDad = true;
					dadHitbox.gravity = 600;
		...
		} else if (currentPlatDad == 28 && doTweenDad && levelDad == 4) {
			doTweenDad = false;
			dadHitbox.gravity = 0;
			new FlxTimer().start(0.05*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(dadHitbox, {y: 340}, 0.21*speedMultiplier, {ease: FlxEase.quadIn});
				new FlxTimer().start(0.21*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					FlxTween.tween(dadHitbox, {y: 428}, 0.25*speedMultiplier, {ease: FlxEase.quadOut});
					new FlxTimer().start(1*speedMultiplier, function(spr:FlxTimer) {
						trace("Done");
						doTweenDad = true;
						dadHitbox.gravity = 600;
		...
		} else if (currentPlatDad == 27 && doTweenDad) {
			doTweenDad = false;
			dadHitbox.gravity = 0;
			new FlxTimer().start(0.05*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(dadHitbox, {y: 404}, 0.21*speedMultiplier, {ease: FlxEase.quadIn});
				new FlxTimer().start(0.21*speedMultiplier, function(spr:FlxTimer) {
					trace("Done");
					FlxTween.tween(dadHitbox, {y: 492}, 0.25*speedMultiplier, {ease: FlxEase.quadOut});
					new FlxTimer().start(0.45*speedMultiplier, function(spr:FlxTimer) {
						trace("Done");
						doTweenDad = true;
						dadHitbox.gravity = 600;
		...
		} else if (currentPlatDad == 31 && doTweenDad) {
			dadHitbox.gravity = 0;
			doTweenDad = false;
			FlxTween.tween(dadHitbox, {y: 487}, 0.07*speedMultiplier, {ease: FlxEase.quadIn});
			new FlxTimer().start(0.07*speedMultiplier, function(spr:FlxTimer) {
				FlxTween.tween(dadHitbox, {y: 433}, 0.28*speedMultiplier, {ease: FlxEase.linear});
				new FlxTimer().start(0.28*speedMultiplier, function(spr:FlxTimer) {
					FlxTween.tween(dadHitbox, {y: 428}, 0.05*speedMultiplier, {ease: FlxEase.quadOut});
					new FlxTimer().start(0.25*speedMultiplier, function(spr:FlxTimer) {
						doTweenDad = true;
						dadHitbox.gravity = 600;
						levelDad = 2;
	...
	*/
	...
	{
		var popup = new objects.FatalPopup(0, 0, type);
	...
	function managePopups(){
		if(FlxG.mouse.justPressed){
			trace("click :)");
			for(idx in 0...objects.FatalPopup.popups.length){
				var realIdx = (objects.FatalPopup.popups.length - 1) - idx;
				var popup = objects.FatalPopup.popups[realIdx];
	...
	override function beatHit()
	{
			...
			case 1:
				...
				notes.forEachAlive(function(note:Note) {
					switch(note.noteType) {
						...
						case 'Bullet Note':
							note.texture = 'arrows/Bullet_Note';
						case 'Knife Note':
							note.texture = 'arrows/Knife_Note';
						case 'Static Note':
							if (noteChangesFromTheStore)
		...
		if (curBeat % 4 == 0 && sunkerTimebarFuckery)
		{
			...
			switch(sunkerTimebarNumber){
				...
				case 5:
					...
					timeBar.updateBar();
				case 6:
					timeBar.createFilledBar(0x000011FF, 0xFF1A2AA8);
					TimeBarColorOne = 0x001A2AA8;
					TimeBarColorTwo = 0xFF1A2AA8;
					if (bgSkin == 'hpR' && ClientPrefs.BarsColor != 'No Color Change') healthBarBG.color = TimeBarColorTwo;
					if (bgTBSkin == 'tbR' && ClientPrefs.BarsColor != 'No Color Change') timeBarBG.color = TimeBarColorTwo;
					timeBar.updateBar();
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && doAutoCamZooms && curBeat % 4 == 0)
		{
			if (camGameZooms)
				FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
