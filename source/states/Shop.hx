...
class Shop extends MusicBeatState
{
	...
	var splashSkins:Array<String> = ['noteSplashes', 'splash-skins/diamond/noteSplashes', 'splash-skins/electric/noteSplashes', 'splash-skins/sparkles/noteSplashes', 'splash-skins/arrow/noteSplashes', 'splash-skins/x-x/noteSplashes', 'splash-skins/squares/noteSplashes'];
	...
	override function create() { //Lets do Some Shit Code :)
		...
		FlxG.sound.playMusic(Paths.music('shop'), 0);
		...
		for (i in 0...splashSkins.length) {
			var splashChoose:FlxSprite = new FlxSprite(545+splashesChooseX, 32+splashesChooseY);
	...
	override function update(elapsed:Float) {
		...
		if (!dontTouchAnything) {
			...
			if ((FlxG.mouse.overlaps(curArrowsSkin) && FlxG.mouse.justPressed) || controls.NOTE_LEFT_P || controls.NOTE_DOWN_P || controls.NOTE_UP_P || controls.NOTE_RIGHT_P) {
				...
				if (curArrowsSkin.animation.curAnim.name == Std.string(selectedArrowPage) + '-pressed' && curArrowsSkin.animation.curAnim.finished) {
					curArrowsSkin.x = 190;
					curArrowsSkin.y = 350;
					curArrowsSkin.animation.play(Std.string(selectedArrowPage) + '-idle', true);
				}
				if (curArrowsSplash.animation.curAnim.name == Std.string(selectedArrowPage) + '-pressed' && curArrowsSplash.animation.curAnim.finished)
					curArrowsSplash.alpha = 0;
			}
