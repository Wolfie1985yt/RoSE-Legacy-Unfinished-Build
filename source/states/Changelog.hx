...
class Changelog extends MusicBeatState
{
	...
	override function create() {
		...
		var info = OpenFlAssets.getText(Paths.json("changelog"));
		for (i in 0...info.split('[-v]').length-1) {
			versions.push(info.split('[-v]')[1+i].split('[v-]')[0]);
			descOrChL.push(info.split('[-c]')[1+i].split('[c-]')[0]);
		}		
		...
		for (i in 0...versions.length) {
			...
			descChL.ID = i;
			descChL.setFormat(Paths.font("advanced.ttf"), 20, 0xFFFF0000, LEFT);
