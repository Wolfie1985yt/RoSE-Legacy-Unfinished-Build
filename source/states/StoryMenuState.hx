...
import objects.StoryModeMenuBFidle as StoryModeMenuBFidle;
					...
					if (!AltSelected) {
						if (ClientPrefs.storyProgressExe != 5) {
							...
							if (!GoreContent) {
								...
								openSubState(new substates.GoreWarningSubstate(false, false, false, 1, true, 'story', 'playstate'));
						...
						if (ClientPrefs.storyProgressAlt != 4) {
							...
							if (!GoreContent) {
								if (songArrayAlt[SelectedSongAlt] == 'coulrophobia') { 
									...
									new FlxTimer().start(1, function(tmr:FlxTimer) { openSubState(new substates.RosyMechanicSubstate()); });
								} else {
									new FlxTimer().start(1, function(tmr:FlxTimer) { Main.InPlaystate = true; LoadingState.loadAndSwitchState(new PlayState()); });
								}
							} else {
								if (songArrayAlt[SelectedSongAlt] == 'coulrophobia') { 
									openSubState(new substates.GoreWarningSubstate(true, false, true, 1, true, 'story', 'rosy-mechanic'));
									GoreWarningMenu = true;
								} else {
									openSubState(new substates.GoreWarningSubstate(false, false, false, 1, true, 'story', 'playstate'));
									GoreWarningMenu = true;
