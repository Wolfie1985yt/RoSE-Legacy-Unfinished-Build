...
class FunkinLua {
	...
	public function new(script:String) {
		...
		if (ClientPrefs.storyProgressExe == 3) {
			set('version', vlc.Cutscenes.psychVersion.trim());
		} else {
		...
		Lua_helper.add_callback(lua, "loadFrames", function(variable:String, image:String, ?SpriteLib:String, ?spriteType:String = "sparrow") {
			var killMe:Array<String> = variable.split('.');
			var spr:FlxSprite = getObjectDirectly(killMe[0]);
			if(killMe.length > 1) {
				spr = getVarInArray(getPropertyLoopThingWhatever(killMe), killMe[killMe.length-1]);
			...
			if(spr != null && image != null && image.length > 0)
			{
				loadFrames(spr, image, SpriteLib, spriteType);
			}
		});
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = false) {
			...
			substates.CustomFadeTransition.nextCamera = PlayState.instance.camOther;
			if(FlxTransitionableState.skipNextTransIn)
				substates.CustomFadeTransition.nextCamera = null;
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float, ?SpriteLib:String) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:ModchartSprite = new ModchartSprite(x, y);
			if(image != null && image.length > 0)
			{
				leSprite.loadGraphic(Paths.image(image, SpriteLib));
			}
			leSprite.antialiasing = ClientPrefs.globalAntialiasing;
			PlayState.instance.modchartSprites.set(tag, leSprite);
			leSprite.active = true;
		});
		Lua_helper.add_callback(lua, "makeAnimatedLuaSprite", function(tag:String, image:String, x:Float, y:Float, ?SpriteLib:String, ?spriteType:String = "sparrow") {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:ModchartSprite = new ModchartSprite(x, y);

			loadFrames(leSprite, image, SpriteLib, spriteType);
			leSprite.antialiasing = ClientPrefs.globalAntialiasing;
			PlayState.instance.modchartSprites.set(tag, leSprite);
		});
			...
			if(PlayState.instance.modchartSprites.exists(tag)) {
				...
				if(!shit.wasAdded) {
					...
					else
					{
						...
						{
							gameovers.GameOverSubstate.instance.insert(gameovers.GameOverSubstate.instance.members.indexOf(gameovers.GameOverSubstate.instance.boyfriend), shit);
						}
						else
						{
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null) {
			...
			#end
			{
				var shit:DialogueFile = objects.DialogueBoxPsych.parseDialogue(path);
	...
	function loadFrames(spr:FlxSprite, image:String, SpriteLib:String, spriteType:String)
	{
		switch(spriteType.toLowerCase().trim())
		{
			...
			default:
				spr.frames = Paths.getSparrowAtlas(image, SpriteLib);
		}
	}
	...
	public function tweenShit(tag:String, vars:String) {
		cancelTween(tag);
		var variables:Array<String> = vars.split('.');
		var sexyProp:Dynamic = getObjectDirectly(variables[0]);
	...
	{
		return PlayState.instance.isDead ? gameovers.GameOverSubstate.instance : PlayState.instance;
	}
}
